...
The scripts are original hapcli scripts. Their workload categories are informed by the public terminal benchmark ecosystem, but no external project source is copied here.

## Run the complete benchmark

Open an hapcli terminal pane in the repository and run one command:
...
```sh
hapcli_BENCHMARK_SIZE_MIB=64 \
hapcli_BENCHMARK_WARMUPS=3 \
hapcli_BENCHMARK_RUNS=5 \
./benchmark/benchmark.sh
```
...
Run it inside a focused hapcli terminal pane, then repeat the same command in each comparison terminal with matching window size, font, scrollback limit, theme, power state, and no competing workload:

```sh
git clone https://github.com/alacritty/vtebench.git
cd vtebench
cargo run --release -- --dat hapcli.dat
```
...
Fixtures are written to `benchmark/.data/` by default and ignored by Git. Set `hapcli_BENCHMARK_DATA_DIR` to place them elsewhere, for example on a RAM disk or a shared benchmarking volume.
