...
impl NativeWasmPluginRuntime {
    ...
    pub async fn call(&mut self, request: PluginRequest) -> Result<PluginResponse, PluginError> {
        self.call_wasm_guest(request, "hapcli_plugin_command")
    }
...
impl NativeWasmPluginInstance {
    ...
    fn drain_outbound_messages(&mut self) -> Result<Vec<PluginOutboundMessage>, PluginError> {
        let Some(drain) = self
            .instance
            .get_typed_func::<(), i64>(&mut self.store, "hapcli_plugin_drain_outbound")
            .ok()
        else {
            return Ok(Vec::new());
        };
    ...
    fn guest_alloc(&mut self, len: usize) -> Result<usize, PluginError> {
        ...
        let alloc = self
            .instance
            .get_typed_func::<i32, i32>(&mut self.store, "hapcli_plugin_alloc")
            .map_err(|error| {
                PluginError::protocol(
                    "wasm_alloc_missing",
                    format!("Native WASM plugin must export hapcli_plugin_alloc: {error}"),
                )
            })?;
