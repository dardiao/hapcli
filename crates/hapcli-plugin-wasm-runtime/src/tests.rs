...
fn unique_temp_dir(name: &str) -> PathBuf {
    let millis = SystemTime::now()
        ...
        .unwrap()
        .as_millis();
    std::env::temp_dir().join(format!("hapcli-wasm-runtime-{name}-{millis}"))
}

fn sample_manifest() -> hapcli_plugin_manifest::NativePluginManifest {
    hapcli_plugin_manifest::NativePluginManifest {
        ...
        locales: None,
        runtime: None,
        permissions: hapcli_plugin_manifest::NativePluginPermissions::default(),
    }
}
    ...
    wat::parse_str(
        r#"
            (module
              ...
              (func (export "_start"))
              (func (export "hapcli_plugin_alloc") (param $len i32) (result i32)
                (local $ptr i32)
                global.get $heap
                local.set $ptr
                global.get $heap
    ...
    let wat = format!(
        r#"
            (module
              ...
              (func (export "hapcli_plugin_command") (param i32 i32) (result i64)
                i64.const 1024
                i64.const 32
                i64.shl
                i64.const {command_len}
                i64.or)
              (func (export "hapcli_plugin_event") (param i32 i32) (result i64)
                i64.const 2048
                i64.const 32
                i64.shl
                i64.const {event_len}
                i64.or)
              (func (export "hapcli_plugin_drain_outbound") (result i64)
                i64.const 3072
                i64.const 32
                i64.shl
                i64.const {drain_len}
