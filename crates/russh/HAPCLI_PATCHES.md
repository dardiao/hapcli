...
and preserve every hapcli-specific compatibility, transfer, and
...
hapcli vendors russh for several independently required behaviors:
...
After changing this vendor fork, run the local russh and hapcli integration
coverage first:
...
```sh
cd /Users/dominical/Documents/hapcli-main/src-tauri
...
2. Check whether upstream now provides equivalent RSA certificate algorithm
   ...
   the upstream implementation when its behavior and tests cover hapcli's
   contract.
