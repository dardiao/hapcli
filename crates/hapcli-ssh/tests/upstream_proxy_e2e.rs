...
async fn local_sshd_upstream_proxy_e2e_matrix() {
    ...
    .await;
    let _socks_env = EnvVarGuard::set("hapcli_SOCKS5_PROXY", String::new());
    let _http_env = EnvVarGuard::set(
        "hapcli_HTTP_PROXY",
        format!("http://{}:{}", http.addr.ip(), http.addr.port()),
    );
    let _no_proxy_env = EnvVarGuard::set("hapcli_NO_PROXY", String::new());
    connect_shell(
...
async fn local_sshd_legacy_scp_file_and_directory_round_trip() {
    ...
    fs::write(&upload_source, b"hapcli SCP file round trip\n").expect("write upload source");
...
fn unique_temp_dir() -> PathBuf {
    let nanos = SystemTime::now()
        ...
        .expect("system clock")
        .as_nanos();
    env::temp_dir().join(format!("hapcli-ssh-e2e-{nanos}"))
}
