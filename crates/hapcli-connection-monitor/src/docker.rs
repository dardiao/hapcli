...
mod tests {
    ...
    fn docker_parser_reads_json_container_rows_without_truncating_names() {
        let output = concat!(
            "===DOCKER===\n",
            "PS\t{\"ID\":\"abc123def4567890\",\"Names\":\"hapcli-cloud-sync\",\"Image\":\"ghcr.io/analyse-decircuit/hapcli-cloud-sync:latest\",\"State\":\"running\",\"Status\":\"Up 7 weeks\",\"Ports\":\"0.0.0.0:8730->8730/tcp\"}\n",
        ...
        assert_eq!(snapshot.status, ResourceDockerStatus::Available);
        assert_eq!(snapshot.containers.len(), 2);
        assert_eq!(snapshot.containers[0].name, "hapcli-cloud-sync");
        assert_eq!(
            snapshot.containers[0].image,
            "ghcr.io/analyse-decircuit/hapcli-cloud-sync:latest"
        );
        assert_eq!(
            snapshot.containers[0].ports.as_deref(),
    ...
    fn docker_parser_uses_inspect_to_replace_truncated_ps_fields() {
        let output = concat!(
            ...
            "INSPECT\t{\"Id\":\"abc123def4567890\",\"Name\":\"/hapcli-cloud-sync\",\"Config\":{\"Image\":\"ghcr.io/analyse-decircuit/hapcli-cloud-sync:latest\"}}\n",
            "===DOCKER_END===",
        );
