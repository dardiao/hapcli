...
const REMOTE_DESKTOP_DIAGNOSTICS_ENV: &str = "hapcli_REMOTE_DESKTOP_DIAGNOSTICS";
...
impl VncDiagnostics {
    ...
    fn log(&self, message: impl AsRef<str>) {
        if self.enabled {
            eprintln!("[hapcli:vnc-helper] {}", message.as_ref());
        }
    }
}
...
fn main() {
    if let Err(error) = run() {
        eprintln!("hapcli-vnc-helper: {error}");
        std::process::exit(1);
    }
}
...
fn run_real_vnc_stdio(reader: &mut impl BufRead) -> Result<(), String> {
    ...
    let worker = thread::Builder::new()
        .name("hapcli-vnc-session".to_string())
        .spawn(move || {
            run_vnc_session(
                session_config,
                request_rx,
