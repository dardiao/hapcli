...
fn acp_agent_preset_migration(
    ...
    match (command, args) {
        ("npx", ["-y", "@agentclientprotocol/claude-agent-acp"]) => {
            Some(("hapcli-native", vec!["--acp-adapter", "claude-code"]))
        }
        ("codex-acp", []) => Some(("hapcli-native", vec!["--acp-adapter", "codex"])),
        // Copilot already exposes native ACP over stdio; undo older hapcli
        // migrations that wrapped it as a text CLI adapter.
        ("hapcli-native", ["--acp-adapter", "github-copilot"])
        | ("hapcli", ["--acp-adapter", "github-copilot"]) => {
...
mod tests {
    ...
    fn migrates_legacy_acp_agent_presets_to_current_launch_commands() {
        ...
        assert_eq!(agents[0].command, "hapcli-native");
        assert_eq!(agents[0].args, vec!["--acp-adapter", "claude-code"]);
        assert_eq!(agents[1].command, "hapcli-native");
