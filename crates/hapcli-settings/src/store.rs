...
pub fn default_settings_path() -> PathBuf {
    if let Ok(Some(data_dir)) = hapcli_portable_runtime::portable_data_dir() {
...
fn default_settings_dir() -> PathBuf {
    if cfg!(windows) {
        if let Some(config_home) = std::env::var_os("APPDATA") {
            return PathBuf::from(config_home).join("hapcli");
        }
    }

    if let Some(home) = std::env::var_os("HOME") {
        return PathBuf::from(home).join(".hapcli");
    }
...
pub fn data_directory_info() -> Result<DataDirectoryInfo> {
    let is_portable = hapcli_portable_runtime::is_portable_mode()
    ...
    let path = if is_portable {
        hapcli_portable_runtime::portable_data_dir()
...
pub fn set_data_directory(path: &Path) -> Result<()> {
    ...
    let test_file = canonical.join(format!(".hapcli_test_{}", std::process::id()));
...
mod tests {
    ...
    fn default_settings_path_matches_tauri_data_directory() {
        ...
        if cfg!(windows) {
            assert!(path.ends_with(Path::new("hapcli").join(SETTINGS_FILENAME)));
        } else {
            assert!(path.ends_with(Path::new(".hapcli").join(SETTINGS_FILENAME)));
        }
    }
    ...
    fn user_visible_data_dir_path_strips_windows_verbatim_prefixes() {
        let disk =
            user_visible_data_dir_path(PathBuf::from(r"\\?\D:\DevSoftWare\Remote\hapcli\data"));
        assert_eq!(
            disk.to_string_lossy(),
            r"D:\DevSoftWare\Remote\hapcli\data"
        );

        let unc = user_visible_data_dir_path(PathBuf::from(r"\\?\UNC\server\share\hapcli"));
        assert_eq!(unc.to_string_lossy(), r"\\server\share\hapcli");
    }
