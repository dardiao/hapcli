mod tests {
    ...
    fn temp_store_path(name: &str) -> PathBuf {
        std::env::temp_dir().join(format!(
            "hapcli-connection-store-{name}-{}.json",
            Uuid::new_v4()
        ))
    }
    ...
    fn encrypted_tauri_connections_payload_round_trips() {
        ...
        assert!(raw.contains("\"format\": \"hapcli.config.encrypted\""));
    ...
    fn managed_key_secret_file_round_trips_large_private_key_material() {
        let data_dir =
            std::env::temp_dir().join(format!("hapcli-managed-key-secret-{}", Uuid::new_v4()));
    ...
    fn managed_key_create_falls_back_to_secret_file_for_large_rsa_keychain_failure() {
        ...
        store.managed_keychain =
            ConnectionKeychain::with_max_secret_bytes_for_tests("com.hapcli.managed-test", 256);
    ...
    fn remote_desktop_profile_update_preserves_options_and_controls_credential_lifecycle() {
        ...
        let initial_options = hapcli_remote_desktop::RemoteDesktopSessionOptions {
            clipboard: hapcli_remote_desktop::RemoteDesktopClipboardOptions {
                text: false,
                images: true,
                files: true,
            },
            audio: hapcli_remote_desktop::RemoteDesktopAudioOptions {
                playback: false,
                capture: true,
            },
            display: hapcli_remote_desktop::RemoteDesktopDisplayOptions {
                use_all_monitors: true,
            },
            vnc: hapcli_remote_desktop::RemoteDesktopVncOptions {
                security_policy:
                    hapcli_remote_desktop::RemoteDesktopVncSecurityPolicy::AllowLegacy,
                session_mode: hapcli_remote_desktop::RemoteDesktopVncSessionMode::Exclusive,
                image_quality:
                    hapcli_remote_desktop::RemoteDesktopVncImageQuality::BestQuality,
                compression: hapcli_remote_desktop::RemoteDesktopVncCompression::High,
            },
        };
        let created = store
            .upsert_remote_desktop_profile(SaveRemoteDesktopProfileRequest {
