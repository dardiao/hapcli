const ENCRYPTED_CONFIG_FORMAT: &str = "hapcli.config.encrypted";
...
const CONFIG_KEYCHAIN_SERVICE: &str = "com.hapcli.config";
const CONFIG_KEYCHAIN_ID: &str = "local-config-master-key";
const MANAGED_SSH_KEY_SECRET_DIR: &str = "managed-ssh-key-secrets";
const MANAGED_SSH_KEY_SECRET_FILE_FORMAT: &str = "hapcli.managed-ssh-key-secret.encrypted";
...
fn load_config_encryption_key() -> Result<Option<ConfigEncryptionKey>> {
    ...
    #[cfg(target_os = "macos")]
    if !hapcli_portable_runtime::is_portable_mode()
...
fn load_config_key_secret() -> Result<Option<zeroize::Zeroizing<String>>> {
    if hapcli_portable_runtime::is_portable_mode()
        .context("failed to determine portable mode")?
    {
        return match hapcli_portable_runtime::keystore::get_secret(
            CONFIG_KEYCHAIN_SERVICE,
            CONFIG_KEYCHAIN_ID,
        ) {
            Ok(secret) => Ok(Some(secret)),
            Err(hapcli_portable_runtime::keystore::PortableKeystoreError::NotFound(_)) => {
                Ok(None)
            }
...
fn store_config_key_secret(secret: &str) -> Result<()> {
    ...
    if hapcli_portable_runtime::is_portable_mode()
        .context("failed to determine portable mode")?
    {
        return hapcli_portable_runtime::keystore::store_secret(
            CONFIG_KEYCHAIN_SERVICE,
            CONFIG_KEYCHAIN_ID,
            secret,
        )
...
fn delete_config_key_secret() -> Result<()> {
    let result = if hapcli_portable_runtime::is_portable_mode()
        .context("failed to determine portable mode")?
    {
        hapcli_portable_runtime::keystore::delete_secret(
            CONFIG_KEYCHAIN_SERVICE,
            CONFIG_KEYCHAIN_ID,
        )
...
fn load_system_config_key_secret() -> Result<Option<zeroize::Zeroizing<String>>> {
    #[cfg(target_os = "macos")]
    hapcli_secret_store::authenticate_device_owner(
        "hapcli needs to unlock your encrypted connections",
    )
    .context("failed to authenticate local config key access")?;

    hapcli_secret_store::NativeSecretStore::new(CONFIG_KEYCHAIN_SERVICE)
