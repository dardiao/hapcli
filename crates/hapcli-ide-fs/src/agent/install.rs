...
fn resolve_agent_binary(target: &str) -> Result<PathBuf, AgentError> {
    let file_name = format!("hapcli-agent-{target}");
...
fn resolve_agent_binary_in_dirs(
    ...
    Err(AgentError::BinaryNotFound(format!(
        "agents/{file_name}; set hapcli_AGENT_DIR or package it in app resources"
    )))
}

fn agent_resource_dirs() -> Vec<PathBuf> {
    let mut dirs = Vec::new();
    if let Some(dir) = std::env::var_os("hapcli_AGENT_DIR") {
    ...
    if let Ok(cwd) = std::env::current_dir() {
        dirs.push(cwd.join("target/agents"));
    }
    dirs
}
