...
mod tests {
    ...
    fn parses_remote_agent_version_like_tauri() {
        ...
        assert_eq!(
            parse_remote_version_output(&format!(
                "hapcli-agent 0.12.1 compat {CURRENT_AGENT_COMPATIBILITY_VERSION}"
            )),
            RemoteAgentInstallState::Current
        );
        assert_eq!(
            parse_remote_version_output("hapcli-agent 0.12.1 compat abc"),
    ...
    fn agent_remote_path_matches_tauri_status_path() {
        assert_eq!(remote_agent_path(), "~/.hapcli/hapcli-agent");
        assert_eq!(
        ...
        assert_eq!(
            shell_path_arg("/home/me/.hapcli/hapcli-agent"),
            "'/home/me/.hapcli/hapcli-agent'"
        );
    }
    ...
    fn resolves_encoded_appimage_agent_payload() {
        ...
        let encoded_path = temp_dir.join("hapcli-agent-aarch64-linux-musl.b64");
        std::fs::write(&encoded_path, "encoded").unwrap();

        let resolved = resolve_agent_binary_in_dirs(
            "hapcli-agent-aarch64-linux-musl",
            vec![temp_dir.clone()],
        )
        .unwrap();
    ...
    async fn reads_encoded_appimage_agent_payload_as_binary() {
        ...
        let encoded_path = temp_dir.join("hapcli-agent-x86_64-linux-musl.b64");
    ...
    fn agent_error_log_labels_do_not_include_remote_payloads() {
        ...
        assert_eq!(
            agent_error_log_label(&AgentError::Ssh(
                "connection failed while running ~/.hapcli/hapcli-agent".to_string(),
            )),
            "ssh"
        );
    }
    ...
    async fn watch_stop_completes_remotely_before_owner_release() {
        let registry = hapcli_ssh::SshConnectionRegistry::default();
        let router = NodeRouter::new(registry.clone());
        let node_id = NodeId::new("node-watch-stop");
        let config = hapcli_ssh::SshConfig::password("watch-host", 22, "me", "pw");
        ...
        registry.mark_state(
            ...
            hapcli_ssh::ConnectionState::Active,
        );
        router
    ...
    async fn ide_session_acquisition_registers_and_releases_ide_consumer() {
        ...
        let config = hapcli_ssh::SshConfig::password("host", 22, "me", "pw");
        router.upsert_node(node_id.clone(), config.clone());

        let handle = registry.acquire(
            config.clone(),
            hapcli_ssh::ConnectionConsumer::NodeRouter("node-ide".to_string()),
        );
        handle.set_physical(Arc::new(()));
        registry.mark_state(handle.connection_id(), hapcli_ssh::ConnectionState::Active);
        router
            .bind_connection(&node_id, handle.connection_id().to_string())
            .unwrap();
    ...
    async fn released_session_rejects_late_connection_acquisition() {
        ...
        let config = hapcli_ssh::SshConfig::password("late-host", 22, "me", "pw");
        router.upsert_node(node_id.clone(), config.clone());
        ...
        registry.mark_state(
            handle.connection_id(),
            hapcli_ssh::ConnectionState::Connecting,
        );
        router
            .bind_connection(&node_id, handle.connection_id().to_string())
            .unwrap();
    ...
    async fn same_node_owners_release_sessions_and_watches_independently() {
        ...
        let config = hapcli_ssh::SshConfig::password("shared-host", 22, "me", "pw");
        router.upsert_node(node_id.clone(), config.clone());
        let handle = registry.acquire(
            config,
            ConnectionConsumer::NodeRouter(node_id.0.clone()),
    ...
    async fn releasing_one_ide_session_preserves_other_node_consumer() {
        ...
        let first_config =
            hapcli_ssh::SshConfig::password("first-host", 22, "first-user", "pw");
        let second_config =
            hapcli_ssh::SshConfig::password("second-host", 22, "second-user", "pw");
    ...
    async fn release_all_ide_consumers_completes_and_releases_registered_consumer() {
        ...
        let handle = registry.acquire(
            config,
            hapcli_ssh::ConnectionConsumer::NodeRouter(node_id.0.clone()),
        );
        handle.set_physical(Arc::new(()));
        registry.mark_state(handle.connection_id(), hapcli_ssh::ConnectionState::Active);
        router
    ...
    async fn ide_remote_session_rebind_releases_previous_connection_consumer() {
        ...
        registry.mark_state(first.connection_id(), hapcli_ssh::ConnectionState::Active);
        router
        ...
        registry.mark_state(first.connection_id(), hapcli_ssh::ConnectionState::LinkDown);
        let second_config = hapcli_ssh::SshConfig::password("host2", 22, "me", "pw");
        ...
        registry.mark_state(second.connection_id(), hapcli_ssh::ConnectionState::Active);
        router
    ...
    async fn ide_session_on_proxy_child_consumes_child_connection() {
        ...
        let parent_config = hapcli_ssh::SshConfig::password("jump", 22, "me", "pw");
        let child_config = hapcli_ssh::SshConfig::password("target", 22, "me", "pw");
        ...
        let parent = registry.acquire(
            parent_config,
            hapcli_ssh::ConnectionConsumer::NodeRouter("jump".to_string()),
        );
        parent.set_physical(Arc::new(()));
        registry.mark_state(parent.connection_id(), hapcli_ssh::ConnectionState::Active);
        router
        ...
        let child = registry.acquire(
            child_config,
            hapcli_ssh::ConnectionConsumer::NodeRouter("target".to_string()),
        );
        child.set_physical(Arc::new(()));
        registry.mark_state(child.connection_id(), hapcli_ssh::ConnectionState::Active);
    ...
    async fn terminal_consumer_release_does_not_kill_ide_remote_fs() {
        ...
        assert!(has_ide_consumer(&handle, "node-ide"));
        assert_eq!(info.state, hapcli_ssh::ConnectionState::Active);
    }
    ...
    async fn parent_link_down_interrupts_child_ide_and_release_cleans_consumer() {
        ...
        assert_eq!(parent.state(), hapcli_ssh::ConnectionState::LinkDown);
        assert_eq!(child.state(), hapcli_ssh::ConnectionState::LinkDown);
        assert!(matches!(
