...
use hapcli_cloud_sync::state::CloudSyncStateStore;
...
fn format_ssh_capability_layer(layer: SshCapabilityLayer) -> &'static str {
    match layer {
        ...
        SshCapabilityLayer::HapcliIntegration => "hapcli-integration",
    }
}
...
mod tests {
    ...
    fn missing_file_is_reported_without_error() {
        let path =
            std::env::temp_dir().join(format!("hapcli-cli-missing-{}", std::process::id()));
