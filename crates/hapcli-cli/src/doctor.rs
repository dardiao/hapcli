...
use hapcli_cloud_sync::state::CloudSyncPersistedState;
use hapcli_connections::{ConnectionStore, SavedUpstreamProxyPolicy};
use hapcli_settings::{
...
fn env_upstream_proxy_source() -> Option<Value> {
    let socks5 = env::var("hapcli_SOCKS5_PROXY").ok();
    if let Some(value) = first_non_empty(socks5.as_deref()) {
        return Some(json!({
            "source": "env",
            "variable": "hapcli_SOCKS5_PROXY",
            "protocol": "socks5",
            "configured": true,
            "hasAuth": value.contains('@'),
            "noProxyConfigured": first_non_empty(env::var("hapcli_NO_PROXY").ok().as_deref()).is_some(),
        }));
    }

    let http = env::var("hapcli_HTTP_PROXY").ok();
    first_non_empty(http.as_deref()).map(|value| {
        json!({
            "source": "env",
            "variable": "hapcli_HTTP_PROXY",
...
#[cfg(test)]
mod tests {
    use hapcli_cloud_sync::StructuredDirtySections;
