...
#[derive(Debug, Parser)]
#[command(name = "hapcli")]
#[command(about = "hapcli headless management CLI")]
#[command(
    long_about = "hapcli headless management CLI for settings, saved connections, portable .oxide bundles, cloud sync, backups, and support diagnostics."
)]
#[command(
    after_help = "Examples:\n  hapcli doctor --strict\n  hapcli settings validate --strict --json\n  hapcli connections search prod\n  hapcli backup create --output ./hapcli-backup.json --json\n  hapcli oxide export ./profile.oxide --connection prod --password-stdin\n  hapcli cloud-sync push --dry-run --json\n  hapcli portable status --json\n  hapcli completion zsh > ~/.zfunc/_hapcli"
)]
pub struct Cli {
    #[arg(
        long,
        global = true,
        value_name = "PATH",
        env = "hapcli_CONFIG_DIR",
        help = "Use PATH as the hapcli config directory"
    )]
...
pub enum Command {
    #[command(about = "Read, validate, import, export, and edit hapcli settings")]
    ...
    #[command(about = "Print resolved hapcli config and data paths")]
    Paths(OutputArgs),
...
#[derive(Debug, Args)]
#[command(
    long_about = "Generate a shell completion script for hapcli. The script is printed to stdout so callers can redirect it to the location required by their shell."
)]
#[command(
    after_help = "Examples:\n  hapcli completion zsh > ~/.zfunc/_hapcli\n  hapcli completion bash > ~/.local/share/bash-completion/completions/hapcli\n  hapcli completion fish > ~/.config/fish/completions/hapcli.fish"
)]
