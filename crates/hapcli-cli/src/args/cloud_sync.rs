...
#[derive(Debug, Args)]
#[command(
    long_about = "Inspect, configure, and operate hapcli cloud sync. Write operations default to dry-run unless confirmed with --yes, and secret commands print only hints/status."
)]
#[command(
    after_help = "Examples:\n  hapcli cloud-sync status --json\n  hapcli cloud-sync configure --backend webdav --endpoint https://example.invalid/sync --dry-run\n  hapcli cloud-sync diff --dirty-only --format table\n  hapcli cloud-sync push --dry-run --json\n  hapcli cloud-sync apply --from remote --strategy merge --yes\n  hapcli cloud-sync secrets set token --stdin"
)]
