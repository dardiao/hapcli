...
#[command(
    after_help = "Examples:\n  hapcli secrets status --scope ai --id builtin-openai --json\n  hapcli secrets set --scope ai --id provider-1 --stdin\n  hapcli secrets set --scope plugin --plugin-id my.plugin --key token --env TOKEN\n  hapcli secrets clear --scope cloud-sync --key token"
)]
