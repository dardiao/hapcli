...
#[derive(Debug, Args)]
#[command(
    long_about = "Read, validate, import, export, and edit hapcli settings without launching the GPUI app. Write commands default to dry-run unless confirmed with --yes."
)]
#[command(
    after_help = "Examples:\n  hapcli settings validate --strict\n  hapcli settings get ai.providers --json\n  hapcli settings set terminal.fontSize 14 --dry-run\n  hapcli settings import ./settings-snapshot.json --section appearance --yes\n  hapcli settings export --section general --json"
)]
