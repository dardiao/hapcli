...
fn parses_connections_show_json() {
    let cli = Cli::parse_from(["hapcli", "connections", "show", "prod", "--json"]);
    match cli.command {
...
fn parses_temporary_ssh_launch() {
    let cli = Cli::parse_from(["hapcli", "ssh", "alice@example.com", "-p", "2222"]);
    match cli.command {
        Command::Ssh(args) => {
...
fn parses_cloud_sync_status() {
    let cli = Cli::parse_from(["hapcli", "cloud-sync", "status", "--json"]);
    match cli.command {
...
fn parses_connections_search() {
    let cli = Cli::parse_from(["hapcli", "connections", "search", "prod", "--json"]);
    match cli.command {
        Command::Connections(command) => match command.action {
...
fn parses_settings_sections() {
    let cli = Cli::parse_from(["hapcli", "settings", "sections", "--json"]);
    match cli.command {
        Command::Settings(command) => match command.action {
...
fn parses_connections_validate() {
    let cli = Cli::parse_from(["hapcli", "connections", "validate", "--strict", "--json"]);
    match cli.command {
        Command::Connections(command) => match command.action {
...
fn parses_top_level_diagnostics() {
    let cli = Cli::parse_from(["hapcli", "diagnose", "--json"]);
    match cli.command {
...
#[test]
fn parses_doctor() {
    let cli = Cli::parse_from(["hapcli", "doctor", "--strict", "--json"]);
    match cli.command {
        Command::Doctor(args) => {
            assert!(args.strict);
            assert!(args.json);
...
fn parses_backup_preview() {
    let cli = Cli::parse_from(["hapcli", "backup", "preview", "--json"]);
    match cli.command {
        Command::Backup(command) => match command.action {
...
fn parses_backup_verify() {
    let cli = Cli::parse_from(["hapcli", "backup", "verify", "backup.json", "--json"]);
    match cli.command {
        Command::Backup(command) => match command.action {
...
fn parses_settings_validate() {
    let cli = Cli::parse_from(["hapcli", "settings", "validate", "--strict", "--json"]);
    match cli.command {
        Command::Settings(command) => match command.action {
...
fn parses_cloud_sync_push_dry_run() {
    let cli = Cli::parse_from(["hapcli", "cloud-sync", "push", "--dry-run", "--json"]);
    match cli.command {
        Command::CloudSync(command) => match command.action {
...
fn parses_cloud_sync_secrets_status() {
    let cli = Cli::parse_from(["hapcli", "cloud-sync", "secrets", "status", "--json"]);
    match cli.command {
        Command::CloudSync(command) => match command.action {
            CloudSyncAction::Secrets(command) => match command.action {
...
fn parses_completion_shell() {
    let cli = Cli::parse_from(["hapcli", "completion", "zsh"]);
    match cli.command {
...
fn parses_completion_install() {
    let cli = Cli::parse_from(["hapcli", "completion", "install", "zsh", "--force"]);
    match cli.command {
...
fn parses_portable_runtime_commands() {
    let status = Cli::parse_from(["hapcli", "portable", "status", "--json"]);
    assert!(matches!(
        status.command,
    ...
    let setup = Cli::parse_from(["hapcli", "portable", "setup", "--password-stdin"]);
    assert!(matches!(
        setup.command,
        Command::Portable(PortableCommand {
            ...
            CloudSyncAction::Backend(command) => match command.action {
                CloudSyncBackendAction::S3(command) => match command.action {
                    CloudSyncBackendConfigureAction::Configure(args) => {
                        assert_eq!(args.s3_bucket.as_deref(), Some("hapcli"));
                        assert_eq!(args.s3_region.as_deref(), Some("us-east-1"));
                        assert!(args.write.dry_run);
                    }
                },
