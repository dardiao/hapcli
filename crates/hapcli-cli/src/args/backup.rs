...
#[derive(Debug, Args)]
#[command(
    long_about = "Create, inspect, verify, and restore local hapcli backups. Restore supports section-limited dry-runs and requires --yes for real writes."
)]
#[command(
    after_help = "Examples:\n  hapcli backup preview --json\n  hapcli backup create --output ./hapcli-backup.json\n  hapcli backup inspect hapcli-backup-123 --summary\n  hapcli backup inspect ./backup.json --section connections --json\n  hapcli backup restore ./backup.json --section settings --dry-run"
)]
