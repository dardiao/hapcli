...
use hapcli_cloud_sync::state::{CloudSyncHistoryEntry, CloudSyncPersistedState};
use hapcli_connections::{ConnectionStore, SavedConnectionsSyncSnapshot};
use hapcli_settings::export_oxide_settings_snapshot_json;
use serde::Serialize;
...
pub(super) const BACKUP_FORMAT: &str = "hapcli-cli-backup-v1";
const BACKUP_FILE_PREFIX: &str = "hapcli-backup-";
...
mod tests {
    ...
    fn backup_names_are_recognized_by_prefix_and_extension() {
        let path = PathBuf::from("hapcli-backup-123.json");
    ...
    fn inspect_query_resolves_plain_file_names_under_backup_dir() {
        let path = resolve_backup_query("hapcli-backup-123");

        assert!(path.ends_with("hapcli-backup-123.json"));
    }
}
