...
use clap::Args;
use hapcli_ssh_launch::{TemporarySshLaunch, parse_user_host_target};
...
#[command(
    after_help = "Examples:\n  hapcli ssh user@example.com\n  hapcli ssh user@example.com -p 2222\n  printf '%s' \"$SSH_PASSWORD\" | hapcli ssh user@example.com --password-stdin"
)]
...
fn build_launch(args: SshLaunchArgs) -> CliResult<TemporarySshLaunch> {
    ...
    let (username, host) = parse_user_host_target(&args.target, default_username.as_deref())
        .map_err(|error| {
            CliError::new(
                "invalid_ssh_target",
                format!("{error}. Use user@host, for example: hapcli ssh alice@example.com"),
                false,
            )
        })?;
...
fn launch_native_gui(request_path: &Path) -> CliResult<()> {
    ...
    #[cfg(not(target_os = "macos"))]
    {
        ...
        Err(CliError::new(
            ...
            "Could not find hapcli-native next to the CLI or in PATH",
            false,
        ))
    }
}
