...
use hapcli_ai::AiProviderKeyStore;
use hapcli_cloud_sync::{secret_keys, secrets::CloudSyncKeychainSecretProvider};
use hapcli_connections::{ConnectionStore, SaveConnectionRequest, SavedAuth, SecretString};
...
fn write_cloud_sync_secret(key: &str, value: Option<&str>, json: bool) -> CliResult<()> {
    ...
    let mut store = hapcli_cloud_sync::state::CloudSyncStateStore::load(path)
        .map_err(|error| runtime_error(error, json))?;
