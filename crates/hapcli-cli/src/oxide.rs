...
use hapcli_forwarding::{ForwardType, OwnedForwardImportRecord, SavedForwardStore};
use hapcli_settings::{
...
fn import(args: OxideImportArgs) -> CliResult<i32> {
    ...
    let mut result = hapcli_connections::oxide_file::apply_oxide_import_with_options(
        &mut store,
        &bytes,
        &password,
        OxideImportOptions {
...
fn export(args: OxideExportArgs) -> CliResult<i32> {
    ...
    let plugin_settings = if args.no_plugin_settings {
        ...
        hapcli_cloud_sync::plugin_settings::load_plugin_settings(&default_settings_path())
    ...
    let quick_commands_json = if args.no_quick_commands {
        ...
        Some(
            hapcli_quick_commands::export_snapshot_json(&default_settings_path())
...
fn quick_command_strategy(
    strategy: crate::args::OxideImportStrategy,
) -> hapcli_quick_commands::QuickCommandImportStrategy {
    match strategy {
        crate::args::OxideImportStrategy::Skip => {
            hapcli_quick_commands::QuickCommandImportStrategy::Skip
        }
        crate::args::OxideImportStrategy::Rename => {
            hapcli_quick_commands::QuickCommandImportStrategy::Rename
        }
        crate::args::OxideImportStrategy::Replace => {
            hapcli_quick_commands::QuickCommandImportStrategy::Replace
        }
...
pub(crate) fn export_portable_secrets(
    json: bool,
) -> CliResult<Vec<hapcli_connections::oxide_file::EncryptedPortableSecret>> {
    let settings = settings::load_settings_read_only(json)?;
    let key_store = hapcli_ai::AiProviderKeyStore::new();
    let provider_ids = hapcli_ai::provider_views(&settings.settings.ai.providers)
        .into_iter()
    ...
    Ok(keys
        .into_iter()
        .map(
            |(id, secret)| hapcli_connections::oxide_file::EncryptedPortableSecret {
...
pub(crate) fn apply_imported_plugin_settings(
    ...
    let filtered = selected_plugin_ids.map_or_else(
        ...
        |selected| {
            plugin_settings
                .iter()
                .filter(|setting| {
                    hapcli_cloud_sync::plugin_settings::plugin_id_from_setting_storage_key(
                        &setting.storage_key,
                    )
    ...
    hapcli_cloud_sync::plugin_settings::upsert_plugin_settings(settings_path, &filtered)
