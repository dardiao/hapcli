...
const CONFIG_DIR_ENV: &str = "hapcli_CONFIG_DIR";
...
pub fn cli_paths() -> CliPaths {
    ...
    let cloud_sync = hapcli_cloud_sync::state::default_cloud_sync_state_path(&settings);
...
fn default_app_settings_dir() -> PathBuf {
    hapcli_settings::default_settings_path()
        .parent()
        .map(PathBuf::from)
