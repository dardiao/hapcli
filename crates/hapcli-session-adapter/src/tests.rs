...
use hapcli_ssh::{AuthMethod, ProxyCommandConfig, UpstreamProxyAuth};
...
fn temp_connection_store(name: &str) -> (ConnectionStore, std::path::PathBuf) {
    let path = std::env::temp_dir().join(format!(
        "hapcli-session-adapter-{name}-{}-connections.json",
        std::process::id()
    ));
...
fn saved_connection(auth: SavedAuth) -> SavedConnection {
    ...
    SavedConnection {
        id: "conn-1".to_string(),
        version: hapcli_connections::CONFIG_VERSION,
        name: "Home".to_string(),
        group: None,
...
fn runtime_settings_conversion_clamps_persisted_values() {
    ...
    assert_eq!(
        terminal_encoding_from_settings(hapcli_settings::TerminalEncoding::Gb18030),
        hapcli_terminal::TerminalEncoding::Gb18030
    );
}
...
fn use_global_upstream_proxy_prefers_global_settings_over_env_fallback() {
    let _socks_env = EnvVarGuard::set("hapcli_SOCKS5_PROXY", "env-proxy.local:1080");
    let _http_env = EnvVarGuard::set("hapcli_HTTP_PROXY", "http://env-http.local:8080");
