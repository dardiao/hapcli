...
impl SshPtySession {
    pub fn new(
        ...
        // 单线程 current_thread runtime：每个 SSH 会话只占 1 个线程，
        // 避免多线程 runtime 按 CPU 核数（M4 为 10）各开 10 个 worker 线程。
                ...
                let result = match connection {
                    ...
                    Some(SshSessionConnection::Existing {
                        ...
                        match (registry, consumer) {
                            ...
                            _ => Err(hapcli_ssh::SshTransportError::ConnectionFailed(
                    ...
                    None => Err(hapcli_ssh::SshTransportError::ConnectionFailed(
