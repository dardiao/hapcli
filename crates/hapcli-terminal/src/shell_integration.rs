...
const HAPCLI_REMOTE_METADATA_OSC: &str = "7719";
...
impl OscCapture {
    ...
    fn push_payload_byte(&mut self, byte: u8) {
        ...
        if self.payload.len() < OSC_LIMIT {
            ...
            self.private = is_private_hapcli_osc(&self.payload);
        }
    }
...
fn is_private_hapcli_osc(payload: &[u8]) -> bool {
    payload
        .strip_prefix(HAPCLI_REMOTE_METADATA_OSC.as_bytes())
...
impl TerminalShellIntegration {
    ...
    fn handle_osc_payload<T: EventListener>(
        ...
        let Ok(text) = std::str::from_utf8(payload) else {
            ...
            return is_private_hapcli_osc(payload);
        };
        let Some((code, data)) = text.split_once(';') else {
            return false;
        };
        if code == HAPCLI_REMOTE_METADATA_OSC {
            match crate::editor_integration::parse_editor_protocol_message(data) {
                ...
                None => {
                    if let Some((cwd, host)) = self.parse_hapcli_remote_metadata(data) {
                        emit(crate::TerminalEvent::CwdChanged { cwd, host });
                    }
                }
            }
            // hapcli private OSC payloads are control metadata and should
    ...
    fn parse_hapcli_remote_metadata(&self, data: &str) -> Option<(String, Option<String>)> {
