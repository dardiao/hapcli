...
const VIM_FREE_TYPE_INTEGRATION_ENV: &str = "hapcli_VIM_INTEGRATION";
const EMACS_FREE_TYPE_INTEGRATION_ENV: &str = "hapcli_EMACS_INTEGRATION";
...
fn prepare_editor_integration(env: &mut HashMap<String, String>) -> Result<LocalShellIntegration> {
    let directory = tempfile::Builder::new()
        .prefix("hapcli-editor-")
        .tempdir()
        .context("create temporary editor integration directory")?;
    let vim_path = directory.path().join("hapcli-free-type.vim");
    let emacs_path = directory.path().join("hapcli-free-type.el");
...
fn prepare_zsh(
    ...
    let zshenv = if config.load_profile {
        ...
        format!(
            "typeset -g __hapcli_user_zdotdir={}\ntypeset -g __hapcli_integration_zdotdir={}\n{}\nsetopt RCS",
            user_zdotdir,
...
fn zsh_source_user_file(name: &str, integration_directory: &Path, final_user_file: bool) -> String {
    ...
    let source = format!(
        "export ZDOTDIR=\"$__hapcli_user_zdotdir\"\nif [[ -n \"${{HISTFILE:-}}\" && \"$HISTFILE\" == \"$__hapcli_integration_zdotdir/\"* ]]; then\n    HISTFILE=\"$ZDOTDIR/${{HISTFILE#\"$__hapcli_integration_zdotdir\"/}}\"\nfi\nif [[ -r \"$ZDOTDIR/{name}\" ]]; then source \"$ZDOTDIR/{name}\"; fi\n__hapcli_user_zdotdir=\"${{ZDOTDIR:-$HOME}}\""
    );
    if final_user_file {
        // Restore the user's effective ZDOTDIR for the interactive session.
        format!(
            "{source}\nexport ZDOTDIR=\"$__hapcli_user_zdotdir\"\nunset __hapcli_user_zdotdir __hapcli_integration_zdotdir"
        )
    } else {
...
__hapcli_emit_cwd() {{
    ...
    command printf '\033]7;file://%s\007' "$(__hapcli_pct_path "$__hapcli_cwd")"
}}
PROMPT_COMMAND="__hapcli_emit_cwd${{PROMPT_COMMAND:+;$PROMPT_COMMAND}}"
__hapcli_emit_cwd"#
    )
}
...
function __hapcli_emit_cwd() {{
    local __hapcli_cwd="${{PWD:A}}"
    command printf '\033]7;file://%s\007' "$(__hapcli_pct_path "$__hapcli_cwd")"
}}
autoload -Uz add-zsh-hook
autoload +X add-zsh-hook
add-zsh-hook precmd __hapcli_emit_cwd
__hapcli_emit_cwd"#
    )
}

fn fish_prompt_hook() -> String {
    r#"function __hapcli_pct_path
    command printf '%s' "$argv[1]" | command od -An -tx1 -v | command tr -d ' \n' | command sed 's/../%&/g; s|%2f|/|g'
end
function __hapcli_emit_cwd --on-event fish_prompt
...
fn nushell_prompt_hook() -> String {
    r#"def __hapcli_pct_path [value: string] {
    ^printf '%s' $value | ^od -An -tx1 -v | ^tr -d ' \n' | ^sed 's/../%&/g; s|%2f|/|g'
}
def __hapcli_emit_cwd [] {
    print --no-newline $"\u{1b}]7;file://(__hapcli_pct_path (pwd))\u{07}"
}
$env.config = ($env.config | upsert hooks.pre_prompt (($env.config.hooks.pre_prompt? | default []) | append {|| __hapcli_emit_cwd }))
__hapcli_emit_cwd"#
        .to_string()
}

fn powershell_prompt_hook() -> String {
    r#"$script:__hapcli_original_prompt = $null
if (Test-Path Function:\prompt) {
    $script:__hapcli_original_prompt = (Get-Command prompt).ScriptBlock
}
function global:__hapcli_pct_path {
    param([string]$Value)
    [Uri]::EscapeDataString($Value)
}
function global:__hapcli_emit_cwd {
    $location = Get-Location
    $cwd = if ($location.ProviderPath) { $location.ProviderPath } else { $location.Path }
    [Console]::Out.Write("`e]7;$(__hapcli_pct_path $cwd)`a")
}
function global:prompt {
    __hapcli_emit_cwd
    if ($script:__hapcli_original_prompt) {
...
mod tests {
        ...
        for shell_id in ["bash", "zsh", "fish", "nu", "pwsh"] {
            ...
            assert!(
                launch
                    .args
                    .iter()
                    .any(|arg| arg.contains("hapcli-shell-"))
    ...
    fn zsh_user_config_resolves_history_from_original_zdotdir() {
        ...
        assert!(
            output.status.success() && stdout.contains("hapcli-history-probe"),
