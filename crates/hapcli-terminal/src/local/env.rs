fn hapcli_terminal_env(config: &LocalPtyConfig, _shell: &ShellInfo) -> HashMap<String, String> {
    let mut terminal_env = HashMap::from([
        ("hapcli_TERM".to_string(), "true".to_string()),
        ("TERM".to_string(), "xterm-256color".to_string()),
        ("TERM_PROGRAM".to_string(), "hapcli".to_string()),
        (
    ...
    // LSCOLORS 自定义为目录蓝、可执行文件（含 .sh）绿。可在设置中关闭。
    if config.shell_colors {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        terminal_env.insert("TERM_PROGRAM".to_string(), "hapcli".to_string());
