mod tests {
    ...
    use hapcli_terminal_graphics::{GraphicsIngress, TerminalGraphicsSegment};
    ...
    fn lsof_cwd_parser_reads_name_record() {
        let lsof_output = "p12345\nn/Users/dominical/Documents/hapcli\n";
        assert_eq!(
            parse_lsof_cwd(lsof_output),
            Some(PathBuf::from("/Users/dominical/Documents/hapcli"))
        );
    }
...
"#;
    ...
    fn integrated_zsh_loads_history_from_user_config_in_a_real_pty() {
        ...
        session.take_events();
        session
            .write_text("print -r -- hapcli_HISTORY_COUNT=${#history[@]}\n")
            .expect("query Zsh history count");

        let deadline = std::time::Instant::now() + Duration::from_secs(5);
        let mut screen = String::new();
        let parse_history_count = |output: &str| {
            output.rsplit("hapcli_HISTORY_COUNT=").find_map(|value| {
                let digits = value
                    .trim_start()
                    .chars()
    ...
    fn vim_startup_clears_previous_shell_content_from_the_alternate_screen() {
        ...
        assert!(
            snapshot.lines[0]
                .cells
                .iter()
                .all(|cell| cell.bg == HAPCLI_DARK_THEME.ansi_background),
    ...
    fn alternate_screen_resize_does_not_restore_primary_screen_backgrounds() {
        ...
        assert!(
            snapshot.lines[0]
                .cells
                .iter()
                .all(|cell| cell.ch == ' ' && cell.bg == HAPCLI_DARK_THEME.ansi_background),
