" hapcli Free Type Mode integration for Vim and Neovim.
" Source this file from vimrc/init.vim only for terminal sessions where the
" editor should expose explicit mode and selection state to hapcli.

if exists('g:loaded_hapcli_free_type')
  finish
endif
let g:loaded_hapcli_free_type = 1
...
function! s:write_terminal(payload) abort
  ...
  let l:private_osc = exists('$hapcli_PRIVATE_OSC') ? $hapcli_PRIVATE_OSC : ''
  let l:shared_multiplexer = (exists('$TMUX') && !empty($TMUX))
        \ || (exists('$STY') && !empty($STY))
        \ || (exists('$ZELLIJ') && !empty($ZELLIJ))
  if l:shared_multiplexer
        \ || (l:private_osc !=# '1' && l:term_program !=# 'hapcli' && l:lc_terminal !=# 'hapcli')
    return
  endif
  try
    if has('nvim')
