...
#[derive(Debug, Parser)]
#[command(name = "hapcli --acp-adapter")]
...
pub fn run_from_env_if_requested() {
    ...
    let cli_args = std::iter::once(OsString::from("hapcli-acp-adapter")).chain(args);
            ...
            async move |message: Dispatch, _connection: ConnectionTo<Client>| match message {
                Dispatch::Request(_, responder) => responder.respond_with_error(
                    agent_client_protocol::Error::method_not_found()
                        .data("hapcli-acp-adapter unsupported ACP method"),
                ),
...
impl AdapterProvider {
    ...
    fn agent_name(self) -> &'static str {
        match self {
            Self::ClaudeCode => "hapcli Claude Code ACP Adapter",
            Self::Codex => "hapcli Codex ACP Adapter",
        }
    }
}
