...
mod tests {
    ...
    fn ai_signatures_ignore_mcp_auth_token() {
        let config = hapcli_ai::McpServerConfig {
            id: "demo".to_string(),
            name: "demo".to_string(),
    ...
    fn acp_agent_presets_create_editable_configs_with_unique_ids() {
        ...
        assert_eq!(claude.command, "hapcli-native");
        assert_eq!(
            claude.args,
        ...
        assert_eq!(codex.command, "hapcli-native");
        assert_eq!(
            codex.args,
...
pub fn ai_mcp_server_signature(
    config: &hapcli_ai::McpServerConfig,
    snapshot: Option<&hapcli_ai::McpServerStateSnapshot>,
) -> u64 {
    let mut hasher = DefaultHasher::new();
...
pub fn ai_mcp_configs(settings: &PersistedSettings) -> Vec<hapcli_ai::McpServerConfig> {
    settings
        .ai
        .mcp_servers
        .iter()
