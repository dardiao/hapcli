...
mod tests {
    ...
    fn infers_powerline_prompt_cwd() {
        let text = "❯ cd hapcli.cloud-sync-server\n\
                    󰊢    ~/hapcli.cloud-sync-server   main !1\n";

        assert_eq!(
            infer_terminal_cwd_from_text(text).as_deref(),
            Some("~/hapcli.cloud-sync-server")
        );
    }

    #[test]
    fn infers_pwd_output_cwd() {
        let text = "❯ pwd\n/home/lipsc/hapcli.cloud-sync-server\n❯";

        assert_eq!(
            infer_terminal_cwd_from_text(text).as_deref(),
            Some("/home/lipsc/hapcli.cloud-sync-server")
        );
    }
