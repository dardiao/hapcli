...
pub fn current_directory_shell_integration_command() -> &'static str {
    "if [ -n \"${BASH_VERSION-}${ZSH_VERSION-}\" ]; then __hapcli_osc7(){ local __oxide_cwd __oxide_host __oxide_path; __oxide_cwd=${PWD:-$(pwd)}; __oxide_host=${HOSTNAME:-$(hostname 2>/dev/null || printf localhost)}; __oxide_path=$(printf '%s' \"$__oxide_cwd\" | awk 'BEGIN{for(i=0;i<256;i++)ord[sprintf(\"%c\",i)]=i}{for(i=1;i<=length($0);i++){c=substr($0,i,1);if(c~/[A-Za-z0-9._~\\/:@-]/)printf \"%s\",c;else printf \"%%%02X\",ord[c]}}'); printf '\\033]7;file://%s%s\\007' \"$__oxide_host\" \"$__oxide_path\"; }; if [ -n \"${ZSH_VERSION-}\" ]; then autoload -Uz add-zsh-hook 2>/dev/null; if typeset -f add-zsh-hook >/dev/null 2>&1; then case \" ${precmd_functions[*]-} \" in *\" __hapcli_osc7 \"*) ;; *) add-zsh-hook precmd __hapcli_osc7 ;; esac; fi; elif [ -n \"${BASH_VERSION-}\" ]; then case \";${PROMPT_COMMAND-};\" in *\";__hapcli_osc7;\"*|\"__hapcli_osc7;\"*) ;; *) PROMPT_COMMAND=\"__hapcli_osc7${PROMPT_COMMAND:+; $PROMPT_COMMAND}\" ;; esac; fi; __hapcli_osc7; fi"
}
...
mod tests {
    ...
    fn parent_handles_home_and_root_paths() {
        ...
        assert_eq!(
            current_directory_parent("~/Documents/hapcli").as_deref(),
            Some("~/Documents")
        );
    ...
    fn shell_integration_command_installs_bash_and_zsh_hooks() {
        ...
        assert!(
            command.contains(
                "PROMPT_COMMAND=\"__hapcli_osc7${PROMPT_COMMAND:+; $PROMPT_COMMAND}\""
            )
        );
        assert!(command.contains("add-zsh-hook precmd"));
        assert!(command.contains("precmd_functions[*]"));
        assert!(command.contains("__hapcli_osc7"));
