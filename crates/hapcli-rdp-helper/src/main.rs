...
const RDP_CLIENT_NAME: &str = "hapcli";
...
const RDP_GRAPHICS_DIAGNOSTICS_ENV: &str = "hapcli_REMOTE_DESKTOP_DIAGNOSTICS";
...
// lets hapcli pass RDP frames to GPUI without an RGBA-to-BGRA channel swap.
...
fn main() {
    if let Err(error) = run() {
        eprintln!("hapcli-rdp-helper: {error}");
        std::process::exit(1);
    }
}
