...
impl RdpGraphicsDiagnostics {
    ...
    fn record_dirty_update(
        ...
        rect: hapcli_remote_desktop::RemoteDesktopRect,
        byte_len: usize,
    ) {
        if !self.enabled {
            return;
    ...
    fn maybe_report(&mut self) {
        ...
        eprintln!(
            "[hapcli:rdp-helper-graphics] trace={} graphics_updates={} skipped={} base_frames={} dirty_updates={} copied_bytes={} base_bytes={} dirty_bytes={} dirty_ratio_per_mille={}",
            self.last_trace_id,
...
async fn wait_for_rdp_authentication(
    ...
    certificate: &hapcli_remote_desktop::RemoteDesktopServerCertificate,
) -> connector::ConnectorResult<()> {
    loop {
...
fn rdp_server_certificate(
...
) -> connector::ConnectorResult<hapcli_remote_desktop::RemoteDesktopServerCertificate> {
    let der = certificate
        .to_der()
    ...
    Ok(hapcli_remote_desktop::RemoteDesktopServerCertificate {
        challenge_id: uuid::Uuid::new_v4().to_string(),
        protocol: RemoteDesktopProtocol::Rdp,
        endpoint: RemoteDesktopEndpoint::new(destination.host(), destination.port()),
        identity_kind: hapcli_remote_desktop::RemoteDesktopServerIdentityKind::X509Certificate,
        ...
        // authoritative identity shown and pinned by hapcli.
        subject: None,
        issuer: None,
        valid_from: None,
        valid_to: None,
...
pub(super) fn attach_client_virtual_channels(
    ...
    // bridges callbacks into hapcli's helper protocol.
        ...
        .map(|monitor| {
            ...
            entry = entry
                .with_orientation(match monitor.orientation {
                    hapcli_remote_desktop::RemoteDesktopMonitorOrientation::Landscape => {
                        MonitorOrientation::Landscape
                    }
                    hapcli_remote_desktop::RemoteDesktopMonitorOrientation::Portrait => {
                        MonitorOrientation::Portrait
                    }
                    hapcli_remote_desktop::RemoteDesktopMonitorOrientation::LandscapeFlipped => {
                        MonitorOrientation::LandscapeFlipped
                    }
                    hapcli_remote_desktop::RemoteDesktopMonitorOrientation::PortraitFlipped => {
...
mod monitor_layout_tests {
    use super::*;
    use hapcli_remote_desktop::{RemoteDesktopMonitor, RemoteDesktopMonitorOrientation};
...
pub(super) fn send_client_rdp_graphics_update(
        ...
        .should_promote_to_base(image)
    {
        ...
        if remote_rdp_helper_graphics_diagnostics_enabled() {
            eprintln!(
                "[hapcli:rdp-helper-graphics] pending_regions={pending_regions} promoted_to_base=true"
            );
        }
        return send_client_rdp_base_frame(output_tx, image, frame_state, true);
    }
...
pub(super) fn rect_pixels(rect: hapcli_remote_desktop::RemoteDesktopRect) -> u64 {
