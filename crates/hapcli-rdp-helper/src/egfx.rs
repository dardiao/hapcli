...
const OPENH264_LIBRARY_PATH_ENV: &str = "hapcli_OPENH264_LIBRARY";
...
pub(super) fn new_egfx_channel(
        ...
        OpenH264Decoder::from_library_path(std::path::Path::new(&path))
            ...
            .map_err(|error| {
                if remote_rdp_helper_graphics_diagnostics_enabled() {
                    eprintln!("[hapcli:rdp-helper-capabilities] OpenH264 unavailable: {error}");
                }
            })
            .ok()
    });
    ...
    let handler = HapcliGraphicsPipelineHandler {
        renderer,
        output_tx,
...
struct HapcliGraphicsPipelineHandler {
    renderer: Arc<Mutex<EgfxRenderer>>,
    output_tx: ClientRdpOutputSender,
...
impl HapcliGraphicsPipelineHandler {
...
impl GraphicsPipelineHandler for HapcliGraphicsPipelineHandler {
...
impl EgfxRenderer {
    ...
    fn confirm_capabilities(&self, capabilities: &CapabilitySet) -> Result<(), String> {
        match capabilities {
            ...
            _ => Err(
                "RDP server confirmed an EGFX capability hapcli did not advertise.".to_string(),
            ),
        }
    }
