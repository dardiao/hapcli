...
use hapcli_quick_commands::QuickCommandsSnapshot;
use hapcli_settings::{PersistedSettings, SettingsStore, export_oxide_settings_snapshot_json};
use serde::Serialize;
...
pub fn build_local_snapshot(
    ...
    let quick_commands_json = hapcli_quick_commands::export_snapshot_json(settings_store.path())
        .map_err(anyhow::Error::msg)?;
    let quick_commands_snapshot: hapcli_quick_commands::QuickCommandsSnapshot =
...
pub fn apply_structured_snapshots(
    ...
    let quick_commands_checkpoint = hapcli_quick_commands::capture_checkpoint(&settings_path)
        .map_err(anyhow::Error::msg)
    ...
    let apply_result = (|| -> Result<CloudSyncApplyOutcome> {
        ...
        let quick_commands_applied = if let Some(snapshot_json) = quick_commands_snapshot_json {
            ...
            let result = hapcli_quick_commands::apply_snapshot_json(
                &settings_path,
                &snapshot_json,
                hapcli_quick_commands::QuickCommandImportStrategy::Merge,
            );
...
fn rollback_structured_apply(
    ...
    connection_checkpoint: &hapcli_connections::ConnectionStoreCheckpoint,
    forwards_checkpoint: Option<&hapcli_forwarding::SavedForwardCheckpoint>,
    quick_commands_checkpoint: &hapcli_quick_commands::QuickCommandsCheckpoint,
    plugin_settings_checkpoint: &plugin_settings::PluginSettingsCheckpoint,
    settings_checkpoint: &hapcli_settings::SettingsStoreCheckpoint,
    ...
    if quick_commands_attempted
        && let Err(error) =
            hapcli_quick_commands::restore_checkpoint(settings_path, quick_commands_checkpoint)
    {
...
fn preflight_structured_snapshots(
    ...
    if let Some(snapshot_json) = quick_commands_snapshot_json {
        let incoming: QuickCommandsSnapshot = serde_json::from_str(snapshot_json)
            .context("failed to decode quick commands snapshot")?;
        let supported_version = hapcli_quick_commands::load_snapshot(settings_store.path())
            .map_err(anyhow::Error::msg)?
            .version;
    ...
    for (section_id, snapshot_json) in app_settings_snapshots {
        ...
        staged_settings = hapcli_settings::merge_oxide_settings_snapshot(
            &staged_settings,
            snapshot_json,
            Some(&selected),
        )?;
...
mod tests {
    ...
    fn settings_stage_failure_restores_settings_and_missing_quick_commands_file() {
        ...
        let quick_snapshot = serde_json::to_string(
            &hapcli_quick_commands::load_snapshot(&settings_path).unwrap(),
        )
        .unwrap();
        let quick_commands_path = hapcli_quick_commands::quick_commands_path(&settings_path);
