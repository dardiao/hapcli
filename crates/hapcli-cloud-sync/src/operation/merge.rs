...
pub(super) fn merge_structured_preview_fields(
    ...
    {
        let local_json = hapcli_quick_commands::export_snapshot_json(settings_store.path())
...
pub(super) fn saved_connection_record_revision(
    record: &hapcli_connections::SavedConnectionSyncRecord,
) -> Result<String> {
    let payload = record
        .payload
        .as_ref()
...
pub(super) fn sync_records_by_id(
    records: &[hapcli_connections::SavedConnectionSyncRecord],
) -> BTreeMap<&str, &hapcli_connections::SavedConnectionSyncRecord> {
    records
        .iter()
...
pub(super) fn forward_records_by_id(
    records: &[hapcli_forwarding::SavedForwardSyncRecord],
) -> BTreeMap<&str, &hapcli_forwarding::SavedForwardSyncRecord> {
    records
        .iter()
        .map(|record| (record.id.as_str(), record))
        .collect()
