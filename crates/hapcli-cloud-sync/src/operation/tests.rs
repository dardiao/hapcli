...
fn connection_sync_record(
    options: hapcli_connections::ConnectionOptions,
) -> hapcli_connections::SavedConnectionSyncRecord {
    hapcli_connections::SavedConnectionSyncRecord {
        id: "conn-1".to_string(),
        revision: "base-revision".to_string(),
        updated_at: "2026-01-01T00:00:00Z".to_string(),
        deleted: false,
        payload: Some(hapcli_connections::ConnectionInfo {
            id: "conn-1".to_string(),
            ...
            auth_type: hapcli_connections::AuthType::Agent,
            key_path: None,
            cert_path: None,
            managed_key_id: None,
            managed_key_name: None,
            proxy_chain: Vec::new(),
            upstream_proxy: hapcli_connections::SavedUpstreamProxyPolicy::UseGlobal,
...
fn remote_desktop_snapshot(
    ...
    host: &str,
) -> hapcli_connections::RemoteDesktopProfilesSyncSnapshot {
...
fn remote_desktop_apply_preserves_only_the_current_devices_credential_ref() {
    let path = std::env::temp_dir().join(format!(
        "hapcli-cloud-sync-remote-desktop-{}.json",
        uuid::Uuid::new_v4()
    ));
    let mut store_data = hapcli_connections::ConnectionStoreData::default();
...
fn connection_merge_preserves_independent_full_option_changes() {
    let base_record = connection_sync_record(hapcli_connections::ConnectionOptions::default());
