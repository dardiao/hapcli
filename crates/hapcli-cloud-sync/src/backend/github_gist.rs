        ...
        let response = execute_cloud_request(
            self.client
                ...
                .body(serde_json::to_vec(&json!({
                    "description": format!("hapcli Cloud Sync ({namespace})"),
                    "public": false,
                    "files": {
                        filename: {
                            "content": "hapcli Cloud Sync storage. Do not edit these files manually.\n",
                        }
                    }
                }))?),
        )
...
fn decode_gist_object_content(content: &str) -> Result<Vec<u8>> {
    ...
    bail!("gist_invalid_encoding: GitHub Gist object is not an hapcli encoded file")
}
...
fn gist_headers(secrets: &CloudSyncSecrets) -> Result<HeaderMap> {
    ...
    headers.insert(USER_AGENT, HeaderValue::from_static("hapcli"));
