...
use hapcli_portable_runtime::keystore::{self as portable_keystore, PortableKeystoreError};
use hapcli_secret_store::NativeSecretStore;
use zeroize::Zeroizing;

use crate::{AuthMode, BackendType, CLOUD_SYNC_PLUGIN_ID, secret_keys};

const CLOUD_SYNC_KEYCHAIN_SERVICE: &str = "com.hapcli.ai";
const LEGACY_NATIVE_CLOUD_SYNC_KEYCHAIN_SERVICE: &str = "com.hapcli.cloud-sync";
    ...
    pub fn store_secret(&mut self, key: &str, value: Option<&str>) -> Result<()> {
        if let Some(value) = value.filter(|value| !value.is_empty()) {
            ...
            if !portable_keychain_enabled()
                .with_context(|| "failed to determine hapcli portable mode")?
            {
...
fn portable_keychain_enabled() -> Result<bool, hapcli_portable_runtime::PortableError> {
    hapcli_portable_runtime::is_portable_mode()
}
...
mod tests {
    ...
    fn plugin_secret_account_id_matches_tauri_namespace() {
        assert_eq!(
            ...
            "plugin-secret:24:com.hapcli.cloud-sync:14:basic-password"
        );
    }
