...
pub fn build_desktop_bootstrap_script(
    ...
    format!(
        r#"#!/bin/bash
# hapcli desktop bootstrap script - auto-generated, do not edit
set -e
...
export XDG_RUNTIME_DIR="/tmp/hapcli-xdg-$$"
...
export DISPLAY={display}
export XDG_RUNTIME_DIR="/tmp/hapcli-app-xdg-$$"
mkdir -p "$XDG_RUNTIME_DIR"
chmod 700 "$XDG_RUNTIME_DIR"
...
echo $$ > /tmp/hapcli-app-$$.pid

cleanup() {{
    rm -f /tmp/hapcli-app-$$.pid
    rm -rf "$XDG_RUNTIME_DIR"
}}
trap cleanup EXIT
...
const PID_FILE: &str = "/tmp/hapcli-desktop.pid";
...
rm -rf /tmp/hapcli-xdg-* 2>/dev/null || true
rm -rf /tmp/hapcli-app-xdg-* 2>/dev/null || true

for pidfile in /tmp/hapcli-app-*.pid; do
    [ -f "$pidfile" ] || continue
...
mod tests {
    ...
    fn desktop_bootstrap_script_matches_tauri_dbus_run_session_path() {
        ...
        assert!(script.contains("echo $$ > /tmp/hapcli-desktop.pid"));
