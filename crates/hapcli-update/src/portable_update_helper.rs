...
pub const PORTABLE_UPDATE_STAGING_DIR: &str = ".hapcli-update-staging";
pub const PORTABLE_UPDATE_BACKUP_DIR: &str = ".hapcli-update-backup";
...
pub fn portable_update_root() -> Result<PathBuf, NativeUpdateError> {
    let info = hapcli_portable_runtime::portable_info().map_err(|error| {
...
fn stage_portable_update(
    ...
    let detached_helper = package_path.with_file_name(format!(
        ".hapcli-update-helper-{process_id}{helper_suffix}"
    ));
...
mod tests {
    ...
    fn manifest() -> PortableUpdateManifest {
        PortableUpdateManifest {
            ...
            app_executable: "hapcli-native".to_string(),
            update_helper: "tools/hapcli-update-helper".to_string(),
            managed_entries: vec![
                "resources".to_string(),
                "tools".to_string(),
                "portable".to_string(),
                PORTABLE_UPDATE_MANIFEST_FILENAME.to_string(),
                "hapcli-native".to_string(),
            ],
        }
    }
    ...
    fn write_staging(staging_dir: &Path, manifest: &PortableUpdateManifest) {
        ...
        fs::write(
            staging_dir.join("tools/hapcli-update-helper"),
            "new helper",
        )
        .unwrap();
        fs::write(staging_dir.join("portable"), "").unwrap();
        fs::write(staging_dir.join("hapcli-native"), "new app").unwrap();
        fs::write(
    ...
    fn helper_arguments_round_trip() {
        let options = PortableUpdateHelperOptions {
            portable_root: PathBuf::from("/opt/hapcli"),
            staging_dir: PathBuf::from("/opt/hapcli/.hapcli-update-staging/payload"),
            backup_dir: PathBuf::from("/opt/hapcli/.hapcli-update-backup"),
            app_exe: PathBuf::from("/opt/hapcli/hapcli-native"),
            wait_pid: Some(42),
            launch_after_apply: true,
        };
        let mut args = vec![OsString::from("hapcli-update-helper")];
    ...
    fn portable_replacement_preserves_user_data_and_unknown_files() {
        ...
        fs::write(root.join("tools/hapcli-update-helper"), "old helper").unwrap();
        fs::write(root.join("portable"), "").unwrap();
        fs::write(root.join("hapcli-native"), "old app").unwrap();
        ...
        let options = PortableUpdateHelperOptions {
            ...
            app_exe: root.join("hapcli-native"),
            wait_pid: None,
        ...
        assert_eq!(
            fs::read_to_string(root.join("hapcli-native")).unwrap(),
            "new app"
        );
        assert_eq!(
    ...
    fn confirmed_portable_update_removes_retained_backup() {
        ...
        fs::write(backup.join("hapcli-native"), "old app").unwrap();
    ...
    fn portable_replacement_rolls_back_applied_entries_after_failure() {
        ...
        let target = root.join("hapcli-native");
        let source = staging_dir.join("hapcli-native");
        ...
        let mut applied = PortableReplacement {
            source,
            target: target.clone(),
            backup: backup_dir.join("hapcli-native"),
            target_existed: false,
            installed: false,
        };
    ...
    fn staging_uses_only_manifest_managed_entries() {
        ...
        let package_path = temp.path().join("hapcli_portable.zip");
        let file = fs::File::create(&package_path).unwrap();
        let mut archive = zip::ZipWriter::new(file);
        let package_root = "hapcli_portable";
        let update_manifest = manifest();
        write_zip_entry(
            &mut archive,
        ...
        write_zip_entry(
            &mut archive,
            &format!("{package_root}/hapcli-native"),
            b"new app",
        );
        write_zip_entry(
            &mut archive,
        ...
        write_zip_entry(
            &mut archive,
            &format!("{package_root}/tools/hapcli-update-helper"),
            b"new helper",
        );
        ...
        assert!(staged.payload_dir.join("hapcli-native").is_file());
