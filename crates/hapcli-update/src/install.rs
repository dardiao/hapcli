...
pub fn plan_native_install(
    ...
    if context.install_flavor == InstallFlavor::Portable {
        return NativeInstallPlan {
            ...
            summary:
                "Portable update staged. hapcli will restart after replacing application files."
                    .to_string(),
        };
    }
    ...
    let (strategy, action, requires_app_exit, summary) =
        match (context.target.os(), context.install_flavor, package_kind) {
            ...
            ("macos", InstallFlavor::MacApp, InstallPackageKind::MacAppBundle) => (
                ...
                true,
                "Schedule .app bundle replacement after hapcli exits.",
            ),
            ("macos", InstallFlavor::MacApp, InstallPackageKind::MacArchive) => (
                InstallStrategy::MacReplaceAppArchive,
                InstallActionKind::LaunchReplacementScript,
                true,
                "Schedule archived .app bundle replacement after hapcli exits.",
            ),
            ...
            ("windows", InstallFlavor::WindowsNsis, InstallPackageKind::WindowsExe) => (
                ...
                InstallActionKind::LaunchInstaller,
                true,
                "Stage the Windows installer and apply it after hapcli exits.",
            ),
            (
                "windows",
    ...
    match plan.package_kind {
        ...
        InstallPackageKind::WindowsExe => {
            launch_windows_installer_via_powershell(
                ...
                &["/S".to_string(), "/hapcli_UPDATE=1".to_string()],
                &plan.package_path,
                true,
                false,
            )?;
...
fn windows_installer_launched_message(
    ...
    if package_kind == InstallPackageKind::WindowsExe {
        return "Windows update staged. hapcli will quit and the update helper will finish replacement."
            .to_string();
    }
    format!(
...
fn execute_linux_appimage_replacement(
    ...
    let install_source = match plan.package_kind {
        ...
        InstallPackageKind::LinuxAppImageArchive => format!(
            r#"tmp_dir="$(mktemp -d "${{TMPDIR:-/tmp}}/hapcli-update.XXXXXX")"
...
fn execute_macos_app_replacement(
    ...
    let install_source = match plan.package_kind {
        ...
        InstallPackageKind::MacArchive => format!(
            r#"tmp_dir="$(/usr/bin/mktemp -d "${{TMPDIR:-/tmp}}/hapcli-update.XXXXXX")"
case "{package}" in
  *.zip|*.app.zip)
...
mod tests {
    ...
    fn portable_updates_use_the_exit_helper() {
        let plan = plan_native_install(
            "/tmp/hapcli_linux_x64_portable.tar.gz",
            &context("linux", InstallFlavor::Portable, "/apps/hapcli-native"),
        );
    ...
    fn windows_installer_is_launched_directly() {
        let plan = plan_native_install(
            ...
            &context(
                "windows",
                InstallFlavor::WindowsNsis,
                "C:/Program Files/hapcli/hapcli.exe",
            ),
        );
    ...
    fn windows_exe_installer_uses_helper_replacement_strategy() {
        let plan = plan_native_install(
            "C:/Temp/hapcli_setup.exe",
            &context(
                "windows",
                InstallFlavor::WindowsNsis,
                "C:/Users/me/AppData/Local/Programs/hapcli/hapcli-native.exe",
            ),
        );
        assert_eq!(plan.strategy, InstallStrategy::WindowsRunInstaller);
    ...
    fn windows_installer_archive_is_extracted_before_launch() {
        let plan = plan_native_install(
            "C:/Temp/hapcli_1.0.0_x64.msi.zip",
            &context(
                "windows",
                InstallFlavor::WindowsNsis,
                "C:/Program Files/hapcli/hapcli.exe",
    ...
    fn windows_start_process_script_uses_runas_and_argument_list() {
        assert_eq!(
            windows_start_process_script(
                ...
                &[
                    "/i".to_string(),
                    "C:/Temp/hapcli Setup.msi".to_string(),
                    "/promptrestart".to_string(),
                ],
                false,
                true,
            ),
            "Start-Process -FilePath 'msiexec.exe' -ArgumentList @('/i', 'C:/Temp/hapcli Setup.msi', '/promptrestart') -Verb RunAs"
        );
    }
    ...
    fn per_user_nsis_update_waits_without_runas() {
        ...
        assert_eq!(
            script,
            "Start-Process -FilePath 'C:/Temp/hapcli Setup.exe' -ArgumentList @('/S', '/hapcli_UPDATE=1') -Wait"
        );
    ...
    fn windows_installer_message_includes_retained_package_path() {
        assert!(
            windows_installer_launched_message(
                Path::new("C:/Temp/hapcli Setup.msi"),
                InstallPackageKind::WindowsMsi,
            )
            .contains("C:/Temp/hapcli Setup.msi")
        );
    }
    ...
    fn macos_dmg_opens_for_finder_installation() {
        let plan = plan_native_install(
            ...
            &context(
                ...
                "/Applications/hapcli.app/Contents/MacOS/hapcli",
            ),
        );
    ...
    fn linux_deb_opens_with_the_package_installer() {
        let plan = plan_native_install(
            "/tmp/hapcli_linux_x64.deb",
            &context(
                "linux",
                InstallFlavor::LinuxDeb,
                "/opt/hapcli/hapcli-native",
            ),
        );
    ...
    fn every_platform_portable_package_uses_the_update_helper() {
        ...
        let cases = [
            ("macos", "/tmp/hapcli_macos_x64_portable.tar.gz"),
            ("windows", "C:/Temp/hapcli_windows_x64_portable.zip"),
            ("linux", "/tmp/hapcli_linux_x64_portable.tar.gz"),
        ];

        for (os, package_path) in cases {
            let plan = plan_native_install(
                package_path,
                &context(os, InstallFlavor::Portable, "/apps/hapcli-native"),
            );
            assert_eq!(plan.strategy, InstallStrategy::PortableReplaceArchive);
            assert_eq!(plan.package_kind, InstallPackageKind::PortableArchive);
            assert!(plan.requires_app_exit);
    ...
    fn linux_pkg_tar_zst_is_a_linux_package() {
        assert_eq!(
            classify_package(Path::new("/tmp/hapcli.pkg.tar.zst")),
            InstallPackageKind::LinuxPackage
        );
    }
