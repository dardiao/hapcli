...
pub const WINDOWS_UPDATE_HELPER_RELATIVE: &str = "tools/hapcli-update-helper.exe";
...
fn windows_restart_manager_app_paths(app_exe: &Path) -> Vec<PathBuf> {
    ...
    if let Some(install_dir) = app_exe.parent() {
        // hapcli 1.x used the Tauri binary name. Register both names so the
        // 2.0 bridge installer can release the old process before replacement.
        app_paths.push(install_dir.join("hapcli.exe"));
    }
    app_paths
}
...
mod tests {
    ...
    fn restart_manager_tracks_native_and_legacy_app_names() {
        let install_dir = PathBuf::from("C:/Users/me/AppData/Local/hapcli");
        assert_eq!(
            windows_restart_manager_app_paths(&install_dir.join("hapcli-native.exe")),
            vec![
                install_dir.join("hapcli-native.exe"),
                install_dir.join("hapcli.exe"),
            ]
        );
    }
    ...
    fn helper_arguments_round_trip() {
        let options = WindowsUpdateHelperOptions {
            install_dir: PathBuf::from("C:/Users/me/AppData/Local/hapcli"),
            app_exe: PathBuf::from("C:/Users/me/AppData/Local/hapcli/hapcli-native.exe"),
            wait_pid: Some(42),
            launch_after_apply: true,
        };
        let mut args = vec![OsString::from("hapcli-update-helper.exe")];
        args.extend(
    ...
    fn staged_update_replaces_files_and_retains_old_files() {
        ...
        fs::write(install_dir.join("hapcli-native.exe"), "old app").unwrap();
        ...
        fs::write(staging.join("hapcli-native.exe"), "new app").unwrap();
        ...
        assert_eq!(
            fs::read_to_string(install_dir.join("hapcli-native.exe")).unwrap(),
            "new app"
        );
        assert_eq!(
        ...
        assert_eq!(
            fs::read_to_string(old_dir.join("hapcli-native.exe")).unwrap(),
            "old app"
        );
        assert_eq!(
    ...
    fn confirmed_update_removes_retained_old_directory() {
        ...
        fs::write(old_dir.join("hapcli-native.exe"), "old app").unwrap();
    ...
    fn staged_update_rolls_back_when_a_move_fails() {
        ...
        let app_target = install_dir.join("hapcli-native.exe");
        let app_source = staging.join("hapcli-native.exe");
        ...
        let mut applied = ReplacementJob {
            ...
            backup: old_dir.join("hapcli-native.exe"),
            target_existed: false,
            installed: false,
        };
