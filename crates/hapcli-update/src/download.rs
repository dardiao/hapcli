...
use hapcli_settings::{UpdateChannel, UpdateProxySettings, is_gpui_preview_version};
use reqwest::{
    StatusCode,
    header::{
...
fn build_update_http_client(
    ...
    let builder = reqwest::Client::builder()
        ...
        .user_agent(format!("hapcli/{}", env!("CARGO_PKG_VERSION")));
    let builder = hapcli_network_proxy::configure_update_http_client_builder(builder, proxy)
...
mod tests {
    ...
    fn package_file_name_keeps_version_and_removes_path_unsafe_chars() {
        let name = package_file_name(&NativeUpdatePackage {
            ...
            url: "https://example.invalid/download/hapcli Preview.dmg?token=secret".into(),
            signature: None,
        });
