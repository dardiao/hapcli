...
struct WasmRuntimeReleaseSupport {
    hapcli_channels: Vec<String>,
    hapcli_versions: Vec<String>,
...
pub(super) fn validate_runtime_index(index: &str, host_version: &str) -> Result<(), String> {
    ...
    let host_channel = current_hapcli_runtime_channel(host_version);
    let compatible = index.runtimes.iter().any(|runtime| {
        runtime
            ...
            && runtime
                .supports
                .plugin_protocol
                .contains(&hapcli_plugin_protocol::NATIVE_PLUGIN_PROTOCOL_VERSION)
            && runtime
                .supports
                .wasm_guest_abi
    ...
    compatible.then_some(()).ok_or_else(|| {
        format!(
            "Latest Wasm runtime does not declare support for hapcli {host_version} ({host_channel})"
        )
    })
}
...
fn current_hapcli_runtime_channel(host_version: &str) -> &'static str {
...
mod tests {
    ...
    const COMPATIBLE_INDEX: &str = r#"{
        "runtimes": [{
            "supports": {
                "hapcliChannels": ["gpui-preview"],
                "hapcliVersions": [">=2.0.0-gpui-preview.0 <3.0.0"],
                "pluginProtocol": [1],
                "wasmGuestAbi": [1],
                "wasi": ["preview1"]
            }
