...
use hapcli_ide_core::{IdePluginFileSnapshot, IdePluginSnapshot};
...
#[cfg(test)]
mod tests {
    use hapcli_ide_core::{IdePluginFileSnapshot, IdePluginProjectSnapshot, IdePluginSnapshot};
    use serde_json::json;
    ...
    fn summary_response_exposes_editor_state_without_names_or_paths() {
        ...
        let response = native_plugin_ide_response(
            hapcli_plugin_protocol::PluginHostCall {
        ...
        let hapcli_plugin_protocol::PluginResponseResult::Ok { value } = response.result else {
