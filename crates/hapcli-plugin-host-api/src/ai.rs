...
pub fn native_plugin_ai_snapshot_value(
    chat: &hapcli_ai::AiChatState,
    providers: &[Value],
    active_provider_id: Option<&str>,
    model_context_windows: &Map<String, Value>,
) -> Value {
    let provider_views = hapcli_ai::provider_views(providers);
    let active_provider = hapcli_ai::active_provider_view(&provider_views, active_provider_id);
...
fn native_plugin_ai_conversation_snapshot(conversation: &hapcli_ai::AiConversation) -> Value {
...
fn native_plugin_ai_message_snapshot(message: &hapcli_ai::AiChatMessage) -> Option<Value> {
    let role = native_plugin_ai_role_label(message.role)?;
    Some(json!({
        "id": &message.id,
        "role": role,
        "content": hapcli_ai::sanitize_for_ai(&message.content),
        "timestamp": message.timestamp_ms,
    }))
}

fn native_plugin_ai_role_label(role: hapcli_ai::AiChatRole) -> Option<&'static str> {
    match role {
        hapcli_ai::AiChatRole::User => Some("user"),
        hapcli_ai::AiChatRole::Assistant => Some("assistant"),
        hapcli_ai::AiChatRole::System => Some("system"),
        hapcli_ai::AiChatRole::Tool => None,
    }
}
