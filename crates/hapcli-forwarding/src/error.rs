...
use hapcli_backend_classification::{BackendErrorClass, classify_io_error_kind};
...
impl From<hapcli_ssh::SshTransportError> for ForwardingError {
    fn from(error: hapcli_ssh::SshTransportError) -> Self {
        match error {
            hapcli_ssh::SshTransportError::ConnectionFailed(message) => {
...
mod tests {
    ...
    fn bind_errors_match_tauri_user_visible_classes() {
        ...
        let remote = ForwardingError::from(hapcli_ssh::SshTransportError::ConnectionFailed(
