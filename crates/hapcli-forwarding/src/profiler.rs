...
use hapcli_ssh::{ConnectionState, ConnectionTransportStatus, SshConnectionHandle};
...
async fn open_profiler_shell(
    ssh_connection: &SshConnectionHandle,
    init_command: &str,
) -> Result<hapcli_ssh::SshShellChannel, hapcli_ssh::SshTransportError> {
    tokio::time::timeout(
        PROFILER_CHANNEL_OPEN_TIMEOUT,
        ssh_connection.open_persistent_shell_channel(init_command),
    )
    .await
    .map_err(|_| hapcli_ssh::SshTransportError::Timeout)?
}
