...
function bugIssue({
  ...
  return {
    number,
    html_url: `https://github.com/AnalyseDeCircuit/hapcli/issues/${number}`,
    title,
    state,
    labels,
...
test('managed comments are idempotent and never claim a fix or release', () => {
  ...
  const existing = {
    id: 42,
    body,
    user: { login: 'hapcli-maintainer[bot]', type: 'Bot' },
  };
  ...
  assert.equal(
    policy.findManagedComment([existing], 'hapcli-maintainer[bot]'),
    existing
  );
});
...
test('blocks automation control files and isolates sensitive product paths', () => {
  const result = policy.classifyChangedPaths([
    ...
    'crates/hapcli-secret-store/src/lib.rs',
    'crates/hapcli-sftp/src/lib.rs',
  ]);
