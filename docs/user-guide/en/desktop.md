...
Open hapcli, then check the left activity bar for the main work areas: sessions, connection pool, connection monitor, Host Tools, graphics/VNC, plugins, cloud sync, file manager, notifications, and settings.
...
hapcli sends ordinary terminal key and text sequences; the remote shell or editor remains the source of truth. Full-screen and mouse-tracking programs keep their own pointer input. Vim, Neovim, and Emacs can additionally expose their current mode and selection through hapcli's explicit adapter, allowing the same Copy/Cut/Paste shortcuts to operate on a verified editor selection without weakening the alternate-screen or mouse protections.

hapcli does not alter editor startup files. To opt in for Vim or Neovim, add this to `vimrc` or `init.vim`:

```vim
if exists('$hapcli_VIM_INTEGRATION') && filereadable($hapcli_VIM_INTEGRATION)
  execute 'source ' . fnameescape($hapcli_VIM_INTEGRATION)
endif
```
...
```lua
local adapter = vim.env.hapcli_VIM_INTEGRATION
...
- Saved profile: what host should hapcli connect to?
