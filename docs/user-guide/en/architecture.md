...
This document describes the hapcli Native system architecture, design decisions, and core components. It follows the same architectural writing style as the Tauri reference: start with principles, show the whole system, separate data and control paths, then describe each subsystem and its lifecycle.
...
6. **Local-first state** - SSH, SFTP, local terminal, settings, plugins, and AI provider configuration work without an hapcli cloud account.
...
| Backend ownership | Core rules and models live in Rust domain crates; `hapcli-gpui-app` owns coordinating Entities, subscriptions, and final runtime shutdown instead of treating them as ad hoc view state |
...
```mermaid
flowchart LR
    User["User"] --> App["hapcli Desktop App"]
    User --> Cli["hapcli CLI Companion"]
...
Owned mainly by `hapcli-gpui-app`, which is both the GPUI surface layer and
...
```text
crates/hapcli-gpui-app/src/workspace.rs
crates/hapcli-gpui-app/src/workspace/tabs/
crates/hapcli-gpui-app/src/workspace/pane_tree.rs
crates/hapcli-gpui-app/src/workspace/sidebar/
crates/hapcli-gpui-app/src/workspace/settings/
```
...
flowchart TB
    ...
    subgraph Domain["Layer 2: Domain Crates"]
        Ssh["hapcli-ssh"]
        Sftp["hapcli-sftp"]
        MonitorDomain["hapcli-connection-monitor"]
        ModemDomain["hapcli-modem-transfer"]
        Ai["hapcli-ai"]
        SettingsDomain["hapcli-settings"]
        PluginsDomain["hapcli-plugin-*"]
        SyncDomain["hapcli-cloud-sync"]
    end
...
- `hapcli-ssh`: SSH configuration, connection registry, node routing, reconnect model.
- `hapcli-sftp`: SFTP protocol/session and transfer semantics.
- `hapcli-connections`: saved connection storage and validation.
- `hapcli-forwarding`: forward rule model.
- `hapcli-ai`: AI providers, context window logic, RAG, MCP, orchestrator tool definitions, policy.
- `hapcli-settings`: settings load/save/mutation logic.
- `hapcli-cloud-sync`: sync and backup logic.
- `hapcli-plugin-*`: plugin manifest, protocol, registry, and host API types.
...
- X/Y/ZMODEM byte-level state lives in `hapcli-modem-transfer`; GPUI only asks for files/directories, displays progress, and writes protocol responses back to the current PTY/channel.
...
- `hapcli-connection-monitor` owns sampler commands, parser logic, row signatures, filters, action command construction, and domain DTOs.
- `hapcli-gpui-app` renders the monitor, confirms user actions, and dispatches commands through the owning node/runtime boundary.
...
- **WSL Graphics**: `hapcli-wsl-graphics` owns WSL distro probing, VNC server startup, desktop/app child processes, and cleanup.
- **Remote Desktop**: `hapcli-remote-desktop` and `hapcli-gpui-remote-desktop` own the shared model, viewer state, and provider registry; `hapcli-rdp-helper` and `hapcli-vnc-helper` own protocol engines behind stdio. RDP reports explicit unsupported errors for integrations outside the current compatibility scope, while VNC uses the direct TCP/RFB path.
- **SSH X11 forwarding**: `hapcli-x11-forwarding` owns DISPLAY, xauth, fake-cookie, setup rewrite, and SSH X11 channel bridge semantics; it is not a full desktop viewer.
...
For RDP/VNC sessions, `hapcli-connections` owns saved profile metadata and secret references, while the app tab/session owns the helper process lifecycle and the GPUI viewer owns framebuffer and input state. RDP/VNC protocol code lives in helper binaries or protocol-specific crates, not in `hapcli-gpui-app`; an SSH node supplies tunnel or node context only when the session is launched through that node.
...
OxideSens is a workspace-aware assistant. It uses configured providers and local app context; it does not require an hapcli account.
...
The `hapcli` CLI companion shares configuration and data paths with the desktop app, but it is not the primary interactive interface.
...
```mermaid
flowchart TB
    Workspace["hapcli-gpui-app<br/>workspace orchestration"] --> AppSurfaces["Workspace Surfaces<br/>sessions · terminal · SFTP · IDE · AI · settings"]

    AppSurfaces --> SshDomain["hapcli-ssh<br/>node routing · registry · reconnect"]
    AppSurfaces --> SftpDomain["hapcli-sftp<br/>sessions · file ops · transfers"]
    AppSurfaces --> MonitorDomain["hapcli-connection-monitor<br/>resources · profilers · host tools"]
    AppSurfaces --> AiDomain["hapcli-ai<br/>providers · tools · policy · context"]
    AppSurfaces --> SettingsDomain["hapcli-settings<br/>settings model · validation"]
    AppSurfaces --> PluginDomain["hapcli-plugin-*<br/>manifest · host API · lifecycle"]
    AppSurfaces --> SyncDomain["hapcli-cloud-sync<br/>preview · backup · apply"]
    AppSurfaces --> TerminalDomain["terminal crates<br/>rendering · PTY · command marks"]
    AppSurfaces --> ModemDomain["hapcli-modem-transfer<br/>X/Y/ZMODEM · protocol state"]
...
| `workspace/acp_workspace.rs` and `hapcli-acp-*` integration | Coordinate ACP agent configuration, sessions, model options, and ACP host-tool bridging beside the native provider path | ACP sessions have an independent agent/session lifecycle and are not ordinary provider streams |
...
|---|---|---|
| `hapcli-ssh/src/config.rs` | Define SSH connection configuration and validation-facing types | Saved metadata is kept separate from live transport handles |
...
| Module | Responsibility | Architectural Boundary |
|---|---|---|
| `hapcli-connection-monitor/src/profiler.rs` | Own sampling cadence, shell setup, timeout limits, and profiler state | Sampling is runtime observation, not saved connection data |
...
| Module | Responsibility | Architectural Boundary |
|---|---|---|
| `hapcli-sftp/src/session.rs` | Own high-level SFTP session behavior | File browsing is tied to a node-level session |
...
|---|---|---|
| Settings | `hapcli-settings`, `hapcli-settings-model`, `hapcli-gpui-settings-view` | Settings are loaded and saved through shared models, then rendered by GPUI pages |
| Saved connections | `hapcli-connections`, session manager modules | Connection records are durable; active nodes are runtime objects |
| Forwarding | `hapcli-forwarding`, app forwarding modules | Rules are configuration; listeners are runtime state |
...
| Terminal modem transfers | `hapcli-modem-transfer`, `hapcli-gpui-terminal` modem worker | Protocol state is terminal-runtime work; file selection and progress are UI concerns |
| Graphics sessions | `hapcli-wsl-graphics`, `hapcli-remote-desktop`, `hapcli-gpui-remote-desktop`, `hapcli-rdp-helper`, `hapcli-vnc-helper`, app graphics/remote-desktop modules | WSL lifecycle, remote protocol helpers, viewer framebuffer, and terminal buffers have separate owners |
| Plugins | `hapcli-plugin-*`, plugin manager and lifecycle modules | Manifests, settings, host API calls, and plugin secrets have separate boundaries |
| Cloud sync | `hapcli-cloud-sync`, `hapcli-gpui-cloud-sync`, app cloud-sync modules | Sync plans, backup creation, and apply steps are explicit control-plane operations |
| Portable runtime | `hapcli-portable-runtime`, settings portable-runtime modules | Portable metadata and encrypted payload handling are separate from normal settings pages |
| Notifications | `hapcli-notification-center`, app notification module | Background status is surfaced as actionable notifications |
| CLI companion | `hapcli-cli` | CLI reads and mutates shared state for automation, but does not replace the desktop workflow |
...
| Area | Crates |
|---|---|
| Desktop shell and workspace glue | `hapcli-gpui-app`, `hapcli-workspace` |
| Shared GPUI components and platform helpers | `hapcli-gpui-ui`, `hapcli-gpui-platform`, `hapcli-theme` |
| Terminal rendering and terminal domain | `hapcli-gpui-terminal`, `hapcli-terminal`, `hapcli-terminal-*` |
| Terminal modem transfers | `hapcli-modem-transfer`, terminal modem worker integration |
| SSH, node routing, reconnect | `hapcli-ssh`, `hapcli-topology`, `hapcli-session-adapter` |
| Host Tools and connection monitoring | `hapcli-connection-monitor`, app connection monitor surface |
| SFTP and transfers | `hapcli-sftp` |
| Saved connections | `hapcli-connections` |
| Forwarding | `hapcli-forwarding`, app forwarding surface |
| Graphics and remote desktop sessions | `hapcli-wsl-graphics`, `hapcli-remote-desktop`, `hapcli-gpui-remote-desktop`, `hapcli-rdp-helper`, `hapcli-vnc-helper`, app graphics/remote-desktop surfaces |
| IDE and editor | `hapcli-gpui-ide`, `hapcli-gpui-editor`, `hapcli-ide-core`, `hapcli-ide-fs`, `hapcli-editor-*` |
| Settings and privilege credentials | `hapcli-settings`, `hapcli-settings-model`, `hapcli-gpui-settings-view`, secret-aware app boundary |
| AI, RAG, MCP, reasoning, and tool policy | `hapcli-ai`, `hapcli-ai-tasks`, `hapcli-skills`, app AI sidebar |
| ACP agent sessions and host tools | `hapcli-acp-adapter`, `hapcli-acp-host-tools`, `workspace/acp_workspace.rs` |
| Plugins | `hapcli-plugin-manifest`, `hapcli-plugin-registry`, `hapcli-plugin-host-api`, `hapcli-plugin-wasm-runtime`, `hapcli-plugin-runtime-install`, app plugin entities |
| Cloud sync and portable runtime | `hapcli-cloud-sync`, `hapcli-gpui-cloud-sync`, `hapcli-portable-runtime` |
| Notifications, launcher, update | `hapcli-notification-center`, `hapcli-launcher`, `hapcli-update` |
| CLI companion | `hapcli-cli` |
