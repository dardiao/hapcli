...
binaries, and the standalone `hapcli` CLI companion. The remote agent uses
...
Open the hapcli desktop app first. The main window is a tabbed SSH workspace with a left activity bar for sessions, files, forwarding, Host Tools, graphics/VNC, plugins, cloud sync, notifications, and settings.
...
```sh
cargo run -p hapcli-cli -- paths
cargo run -p hapcli-cli -- doctor --strict
```
...
```sh
hapcli --config-dir ./fixtures/profile-a paths
hapcli_CONFIG_DIR=./fixtures/profile-a hapcli doctor --strict
```

Use named profiles when one config root must hold several isolated profiles:

```sh
hapcli --config-dir ./fixtures --profile staging paths
```
...
```sh
hapcli settings set terminal.fontSize 14 --dry-run --json
hapcli settings set terminal.fontSize 14 --yes
```
