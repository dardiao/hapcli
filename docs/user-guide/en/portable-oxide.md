...
`.oxide` bundles are encrypted portable exports for moving hapcli data between machines or profiles. They can include connections, forwards, app settings, quick commands, plugin settings, managed SSH keys, and optionally portable secrets.
...
- Whether managed SSH keys are present and whether they will be restored into hapcli.
...
- hapcli-managed SSH keys can be included so managed-key connections restore to the managed key store.
...
Managed keys are hapcli-owned credentials. A connection stores only a managed key reference, while the private key material stays in the local keychain or portable keystore.

When a bundle contains managed keys, the import preview lets you restore them into hapcli. If restore is disabled, hapcli may use embedded fallback key files when available; otherwise affected connections need auth repair after import.
...
```sh
hapcli oxide validate ./profile.oxide
hapcli oxide preview-import ./profile.oxide --password-stdin --json
hapcli oxide diff ./profile.oxide --strategy merge --password-env OXIDE_PASSWORD
hapcli portable status --json
```
