...
本文档描述 hapcli Native 的系统架构、设计决策和核心组件。写法遵循 Tauri 参考文档的结构：先说明设计理念，再给出整体架构图，随后按通信平面、分层、核心组件和生命周期展开。
...
6. **本地优先状态** - SSH、SFTP、本地终端、设置、插件和 AI 供应商配置不依赖 hapcli 云账号。
...
| 后端所有权 | 核心规则和模型位于 Rust 领域 crate；`hapcli-gpui-app` 持有协调 Entity、订阅和最终运行时关闭逻辑，而不是把它们当作临时界面状态 |
...
```mermaid
flowchart LR
    User["用户"] --> App["hapcli 桌面应用"]
    User --> Cli["hapcli CLI 伴侣工具"]
...
主要由 `hapcli-gpui-app` 承担；它既是 GPUI 页面层，也是长期应用实体的协调器。
...
- `hapcli-ssh`：SSH 配置、连接注册表、节点路由、重连模型。
- `hapcli-sftp`：SFTP 协议、会话和传输语义。
- `hapcli-connections`：保存连接存储和校验。
- `hapcli-forwarding`：转发规则模型。
- `hapcli-ai`：AI 供应商、上下文窗口、RAG、MCP、orchestrator 工具定义和策略。
- `hapcli-settings`：设置加载、保存和变更逻辑。
- `hapcli-cloud-sync`：同步和备份逻辑。
- `hapcli-plugin-*`：插件清单、协议、注册表和宿主 API 类型。
...
- X/Y/ZMODEM 字节级状态由 `hapcli-modem-transfer` 持有；GPUI 只负责请求文件/目录、显示进度，并把协议响应写回当前 PTY 或通道。
...
- `hapcli-connection-monitor` 持有采样命令、解析逻辑、行签名、过滤器、动作命令构造和领域 DTO。
- `hapcli-gpui-app` 负责渲染监控页面、确认用户动作，并通过所属节点/运行时边界分发命令。
...
- **WSL 图形**：`hapcli-wsl-graphics` 拥有 WSL 发行版探测、VNC server 启动、桌面/应用子进程和清理。
- **远程桌面**：`hapcli-remote-desktop` 和 `hapcli-gpui-remote-desktop` 负责共享模型、查看器状态和 provider registry；`hapcli-rdp-helper` 与 `hapcli-vnc-helper` 通过 stdio 边界负责协议引擎。RDP 对当前兼容范围之外的集成返回明确的 unsupported 错误，VNC 使用直接 TCP/RFB 路径。
- **SSH X11 转发**：`hapcli-x11-forwarding` 拥有 DISPLAY、xauth、假 cookie、setup rewrite 和 SSH X11 channel 桥接语义；它不是完整桌面查看器。
...
对于 RDP/VNC 会话，`hapcli-connections` 拥有保存配置元数据和凭据引用，应用 tab/session 拥有 helper 进程生命周期，GPUI 查看器拥有 framebuffer 和输入状态。RDP/VNC 协议代码放在 helper binary 或协议专属 crate 中，而不是放进 `hapcli-gpui-app`；只有通过节点启动时，SSH 节点才提供 tunnel 或节点上下文。
...
OxideSens 是理解工作区上下文的助手。它使用已配置供应商和本地应用上下文，不需要 hapcli 账号。
...
`hapcli` CLI 伴侣工具与桌面应用共享配置和数据路径，但不是主要交互界面。
...
```mermaid
flowchart TB
    Workspace["hapcli-gpui-app<br/>工作区编排"] --> AppSurfaces["工作区页面<br/>会话 · 终端 · SFTP · IDE · AI · 设置"]

    AppSurfaces --> SshDomain["hapcli-ssh<br/>节点路由 · 注册表 · 重连"]
    AppSurfaces --> SftpDomain["hapcli-sftp<br/>会话 · 文件操作 · 传输"]
    AppSurfaces --> MonitorDomain["hapcli-connection-monitor<br/>资源 · profiler · 主机工具"]
    AppSurfaces --> AiDomain["hapcli-ai<br/>供应商 · 工具 · 策略 · 上下文"]
    AppSurfaces --> SettingsDomain["hapcli-settings<br/>设置模型 · 校验"]
    AppSurfaces --> PluginDomain["hapcli-plugin-*<br/>manifest · 宿主 API · 生命周期"]
    AppSurfaces --> SyncDomain["hapcli-cloud-sync<br/>预览 · 备份 · 应用"]
    AppSurfaces --> TerminalDomain["终端 crate<br/>渲染 · PTY · 命令标记"]
    AppSurfaces --> ModemDomain["hapcli-modem-transfer<br/>X/Y/ZMODEM · 协议状态"]
...
| `workspace/acp_workspace.rs` 与 `hapcli-acp-*` 集成 | 协调 ACP agent 配置、session、模型选项和 ACP 主机工具桥接，并与原生供应商路径并列 | ACP session 有独立的 agent/session 生命周期，不应被当作普通供应商流 |
...
| 模块 | 职责 | 架构边界 |
|---|---|---|
| `hapcli-ssh/src/config.rs` | 定义 SSH 连接配置和面向校验的类型 | 保存元数据与在线传输句柄分离 |
...
| 模块 | 职责 | 架构边界 |
|---|---|---|
| `hapcli-connection-monitor/src/profiler.rs` | 持有采样节奏、shell 初始化、超时限制和 profiler 状态 | 采样是运行时观察，不是保存连接数据 |
...
| 模块 | 职责 | 架构边界 |
|---|---|---|
| `hapcli-sftp/src/session.rs` | 持有高层 SFTP 会话行为 | 文件浏览绑定到节点级会话 |
...
| 区域 | Native 所有者 | 说明 |
|---|---|---|
| 设置 | `hapcli-settings`, `hapcli-settings-model`, `hapcli-gpui-settings-view` | 设置通过共享模型加载和保存，再由 GPUI 页面渲染 |
| 保存连接 | `hapcli-connections`, 会话管理器模块 | 连接记录是持久对象；活跃节点是运行时对象 |
| 转发 | `hapcli-forwarding`, 应用转发模块 | 规则是配置；监听器是运行时状态 |
| 提权凭据 | 设置提权页面、终端提权提示、凭据感知存储 | 作用域和提示匹配器是配置；secret 值留在普通设置之外 |
| 终端 modem 传输 | `hapcli-modem-transfer`, `hapcli-gpui-terminal` modem worker | 协议状态属于终端运行时；文件选择和进度属于 UI |
| 图形会话 | `hapcli-wsl-graphics`, `hapcli-remote-desktop`, `hapcli-gpui-remote-desktop`, `hapcli-rdp-helper`, `hapcli-vnc-helper`, 应用图形/远程桌面模块 | WSL 生命周期、远程协议 helper、viewer framebuffer 和终端缓冲区各自归属清晰 |
| 插件 | `hapcli-plugin-*`, 插件管理器和生命周期模块 | manifest、设置、宿主 API 调用和插件凭据有各自边界 |
| 云同步 | `hapcli-cloud-sync`, `hapcli-gpui-cloud-sync`, 应用云同步模块 | 同步计划、备份创建和应用步骤都是显式控制平面操作 |
| 便携运行时 | `hapcli-portable-runtime`, 设置中的便携运行时模块 | 便携元数据和加密载荷处理与普通设置页分离 |
| 通知 | `hapcli-notification-center`, 应用通知模块 | 后台状态以可操作通知呈现 |
| CLI 伴侣工具 | `hapcli-cli` | CLI 为自动化读取和修改共享状态，但不替代桌面工作流 |
...
| 区域 | Crates |
|---|---|
| 桌面外壳和工作区衔接 | `hapcli-gpui-app`, `hapcli-workspace` |
| 共享 GPUI 组件和平台辅助 | `hapcli-gpui-ui`, `hapcli-gpui-platform`, `hapcli-theme` |
| 终端渲染和终端领域 | `hapcli-gpui-terminal`, `hapcli-terminal`, `hapcli-terminal-*` |
| 终端 modem 传输 | `hapcli-modem-transfer`, 终端 modem worker 集成 |
| SSH、节点路由、重连 | `hapcli-ssh`, `hapcli-topology`, `hapcli-session-adapter` |
| 主机工具和连接监控 | `hapcli-connection-monitor`, 应用连接监控页面 |
| SFTP 与传输 | `hapcli-sftp` |
| 保存连接 | `hapcli-connections` |
| 转发 | `hapcli-forwarding`, 应用转发页面 |
| 图形和远程桌面会话 | `hapcli-wsl-graphics`, `hapcli-remote-desktop`, `hapcli-gpui-remote-desktop`, `hapcli-rdp-helper`, `hapcli-vnc-helper`, 应用图形/远程桌面页面 |
| IDE 和编辑器 | `hapcli-gpui-ide`, `hapcli-gpui-editor`, `hapcli-ide-core`, `hapcli-ide-fs`, `hapcli-editor-*` |
| 设置和提权凭据 | `hapcli-settings`, `hapcli-settings-model`, `hapcli-gpui-settings-view`, 应用凭据感知边界 |
| AI、RAG、MCP、推理和工具策略 | `hapcli-ai`, `hapcli-ai-tasks`, `hapcli-skills`, 应用 AI 侧边栏 |
| ACP agent session 和主机工具 | `hapcli-acp-adapter`, `hapcli-acp-host-tools`, `workspace/acp_workspace.rs` |
| 插件 | `hapcli-plugin-manifest`, `hapcli-plugin-registry`, `hapcli-plugin-host-api`, `hapcli-plugin-wasm-runtime`, `hapcli-plugin-runtime-install`, 应用插件 Entity |
| 云同步和便携运行时 | `hapcli-cloud-sync`, `hapcli-gpui-cloud-sync`, `hapcli-portable-runtime` |
| 通知、启动器、更新 | `hapcli-notification-center`, `hapcli-launcher`, `hapcli-update` |
| CLI 伴侣工具 | `hapcli-cli` |
