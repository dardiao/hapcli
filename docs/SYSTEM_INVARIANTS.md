# hapcli System Invariants

This document records rules that must stay true while hapcli evolves. These
...
- GPUI forms must compose shared hapcli UI primitives for text input,
...
- When translating the Tauri UI, first identify the source token or Tailwind
  class, then map it to an hapcli token. Do not eyeball values from a
...
- Vim, Neovim, and Emacs adapters may enable the application's own mouse
  protocol, but hapcli must not remove the generic alternate-screen or
...
- Native GPUI does not expose a product-level CPU renderer fallback in the
  current GPUI version. hapcli must not present WebGL, Canvas, or CPU
  renderer choices as if they were real native renderers.
- Renderer compatibility settings must be expressed as an hapcli render
...
- Safe-mode environment overrides such as `hapcli_RENDER_PROFILE` apply only
...
- hapcli native code must not copy, mechanically translate, or closely mirror
...
- Source comments and identifiers should use hapcli terminology, not Zed
...
- UI token changes require at least `cargo check -p hapcli-native`.
